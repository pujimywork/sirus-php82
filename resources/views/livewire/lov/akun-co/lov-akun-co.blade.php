...
/**
 * LOV Akun Cash-Out (CO) — untuk pengeluaran kas lainnya.
 * Query: acmst_accounts where active_status='1' and co_status='1'
 */
...
new class extends Component {
    ...
    public string $label = 'Akun Pengeluaran';
            ...
            <div class="absolute z-50 w-full mt-2 overflow-hidden bg-white border border-gray-200 shadow-lg rounded-xl dark:bg-gray-900 dark:border-gray-700">
                <ul class="overflow-y-auto divide-y divide-gray-100 max-h-72 dark:divide-gray-800">
                    @foreach ($options as $index => $option)
                        <li wire:key="lov-co-{{ $option['acc_id'] ?? $index }}" x-ref="lovItem{{ $index }}">
