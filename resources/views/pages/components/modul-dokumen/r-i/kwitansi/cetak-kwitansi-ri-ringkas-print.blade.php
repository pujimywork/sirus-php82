<x-pdf.layout-a4-with-out-background title="PERINCIAN BIAYA PERAWATAN DAN PENGOBATAN RAWAT INAP">
    ...
    {{-- ══════════════════════════════════════
         MAIN TABLE — 3 kolom konsisten:
         (1) label / detail | (2) nominal item | (3) subtotal section
    ══════════════════════════════════════ --}}
    <table width="100%" cellpadding="0" cellspacing="0" class="text-[10px] mt-2 border-collapse">

        {{-- ─── A. BIAYA KAMAR ─── --}}
        <tr><td colspan="3" class="{{ $hdrSection }}">A. BIAYA KAMAR</td></tr>
        <tr>
        ...
        <tr>
            <td colspan="2" class="{{ $cellSubTot }}"></td>
            <td class="{{ $cellSubTot }} w-[110px]">{{ $rp($data['aTotal']) }}</td>
        </tr>

        {{-- ─── B. JASA MEDIS ─── --}}
        <tr><td colspan="3" class="{{ $hdrSection }}">B. JASA MEDIS</td></tr>
        <tr>
            <td class="{{ $cellLabel }}">DOKTER</td>
            <td class="{{ $cellAmt }} w-[110px]">{{ $rp($data['bDokter']) }}</td>
            <td class="w-[110px]"></td>
        </tr>
        <tr>
            <td class="{{ $cellLabel }}">TINDAKAN DOKTER</td>
            <td class="{{ $cellAmt }}">{{ $rp($data['bTindakanDokter']) }}</td>
            <td></td>
        </tr>
        <tr>
            <td class="{{ $cellLabel }}">TINDAKAN MEDIS</td>
            <td class="{{ $cellAmt }}">{{ $rp($data['bTindakanMedis']) }}</td>
            <td></td>
        </tr>
        <tr>
            <td class="{{ $cellLabel }}">JASA PERAWATAN</td>
            <td class="{{ $cellAmt }}">{{ $rp($data['bJasaPerawatan']) }}</td>
            <td></td>
        </tr>
        <tr>
            <td class="{{ $cellLabel }}">PELAYANAN UMUM</td>
            <td class="{{ $cellAmt }}">{{ $rp($data['bPelayananUmum']) }}</td>
            <td></td>
        </tr>
        <tr>
            <td colspan="2" class="{{ $cellSubTot }}"></td>
        ...
        <tr><td colspan="3" class="{{ $hdrSection }}">C. PENUNJANG DIAGNOSTIK</td></tr>
        <tr>
        ...
        {{-- ─── D. PEMAKAIAN OBAT ─── --}}
        <tr><td colspan="3" class="{{ $hdrSection }}">D. PEMAKAIAN OBAT</td></tr>
        <tr>
        ...
        @if ($data['eOperasi'] > 0)
        <tr><td colspan="3" class="{{ $hdrSection }}">E. OPERASI</td></tr>
        <tr>
        ...
        <tr><td colspan="3" class="{{ $hdrSection }}">F. ADMINISTRASI DAN LAIN-LAIN</td></tr>
        <tr>
            <td class="{{ $cellLabel }}">ADMINISTRASI</td>
            <td class="{{ $cellAmt }}">{{ $rp($data['fAdministrasi']) }}</td>
            <td></td>
        </tr>
        <tr>
            <td class="{{ $cellLabel }}">STATUS</td>
            <td class="{{ $cellAmt }}">{{ $rp($data['fStatus']) }}</td>
            <td></td>
        </tr>
        <tr>
        ...
        <tr>
            <td class="{{ $cellLabel }} uppercase">
                {{ $oth->desc }} <span class="text-gray-600">({{ $oth->qty }} X)</span>
            </td>
        ...
        <tr><td colspan="3" class="{{ $hdrSection }}">G. RETURN OBAT</td></tr>
        <tr>
        ...
        {{-- ─── FOOTER ─── --}}
        <tr><td colspan="3" class="p-0 border-t border-gray-800"></td></tr>
        <tr>
            <td colspan="2" class="py-0.5 px-1.5 text-right font-semibold">TOTAL BIAYA</td>
            <td class="py-0.5 px-1.5 text-right tabular-nums font-semibold">{{ $rp($data['subtotal']) }}</td>
        </tr>
        @if ($data['resepLunasFooter'] > 0)
        <tr>
            <td colspan="2" class="py-px px-1.5 text-right">RESEP LUNAS (dibayar di apotek)</td>
            <td class="py-px px-1.5 text-right tabular-nums">( {{ $rp($data['resepLunasFooter']) }} )</td>
        </tr>
        @endif
        @if ($data['subsidi'] > 0)
        <tr>
            <td colspan="2" class="py-px px-1.5 text-right">SUBSIDI</td>
            <td class="py-px px-1.5 text-right tabular-nums">( {{ $rp($data['subsidi']) }} )</td>
        </tr>
        @endif
        @if ($data['sudahBayar'] > 0)
        <tr>
            <td colspan="2" class="py-px px-1.5 text-right">SUDAH DIBAYAR</td>
            <td class="py-px px-1.5 text-right tabular-nums">( {{ $rp($data['sudahBayar']) }} )</td>
        </tr>
        @if ($data['sisa'] > 0)
        <tr>
            <td colspan="2" class="py-0.5 px-1.5 text-right font-semibold border-t border-gray-400">SISA TAGIHAN</td>
