{{-- resources/views/pages/components/rekam-medis/u-g-d/etiket-obat/cetak-etiket-obat-print.blade.php --}}
<x-pdf.layout-etiket>
    ...
    <table class="w-full pb-1 mb-1 border-b border-gray-400" cellpadding="0" cellspacing="0">
        <tr>
            ...
            <td class="text-left align-middle">
                ...
                <div class="text-gray-500" style="font-size:5.5pt;">Unit Gawat Darurat</div>
            </td>
        </tr>
    </table>
    ...
    <table class="w-full" cellpadding="0" cellspacing="0">
        <tr>
            ...
            <td class="text-[8.5px] font-bold text-black align-top py-[0.2mm]">
                {{ $item->reg_name ?? '-' }} / {{ ($item->sex ?? '') === 'L' ? 'L' : (($item->sex ?? '') === 'P' ? 'P' : '-') }}
            </td>
        </tr>
        <tr>
    ...
    <table class="w-full my-1 py-1 border-t border-b border-dashed border-gray-400" cellpadding="0" cellspacing="0">
        ...
        <tr>
            <td class="pl-1 text-[7px] font-bold text-black align-middle">
                {{ $item->ugd_carapakai ?? '-' }} X SEHARI
                @if (!empty($item->ugd_kapsul))
                    {{ $item->ugd_kapsul }}
                @endif
                @if (!empty($item->ugd_takar))
                    {{ $item->ugd_takar }}
                @endif
                @if (!empty($item->ugd_ket))
                    <span class="font-normal text-gray-600">({{ $item->ugd_ket }})</span>
                @endif
