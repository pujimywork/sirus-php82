<?php
// Viewer read-only "Pengkajian Akhir Hayat" — display Rekam Medis RI.
// Pembeda entri = id (uuid). Payload cetak bespoke (entry + opsiLabel + clause),
// jadi tak memakai streamCetakDokumenRi generik.
...
new class extends Component {
    ...
    private string $printView = 'pages.components.modul-dokumen.r-i.akhir-hayat-ri.cetak-akhir-hayat-ri-print';
    ...
    /** Payload cetak identik dgn aksi cetak() di komponen EMR Akhir Hayat. */
    ...
    public function lihat(string $id): void
    {
        ...
        $this->dispatch('open-modal', name: "view-akhir-hayat-ri-{$this->riHdrNo}");
    }
    ...
    public function cetak(string $id): mixed
    {
        ...
        return response()->streamDownload(fn() => print $pdf->output(), 'akhir-hayat-ri-' . ($data['regNo'] ?? $this->riHdrNo) . '.pdf');
    }
};
?>
...
<div>
    ...
    <x-rm.dokumen-view-modal name="view-akhir-hayat-ri-{{ $riHdrNo }}" title="Pengkajian Akhir Hayat"
