<?php
// Viewer read-only "Pengkajian Pra-Operasi" — display Rekam Medis RI.
...
<div>
    <x-border-form title="Pengkajian Pra-Operasi">
        @forelse (collect($list)->filter(fn($entri) => filled(data_get($entri, 'createdAt')))->values() as $entri)
            <x-rm.doc-list-row :id="data_get($entri, 'createdAt')" :title="data_get($entri, 'rencanaOperasi') ?: 'Pengkajian Pra-Operasi'"
                :date="data_get($entri, 'tanggalOperasi')" :sub="'Operator: ' . (data_get($entri, 'dokterOperator') ?: '-')" />
        @empty
            <x-rm.doc-empty />
        @endforelse
    </x-border-form>

    <x-rm.dokumen-view-modal name="view-pengkajian-pre-op-ri-{{ $riHdrNo }}" title="Pengkajian Pra-Operasi"
