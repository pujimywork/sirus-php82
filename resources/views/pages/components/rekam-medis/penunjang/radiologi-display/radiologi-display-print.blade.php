...
<x-pdf.layout-a4-with-out-background title="HASIL PEMERIKSAAN RADIOLOGI">
    ...
    <x-slot name="patientData">
        @php
            ...
            $tglPeriksa = $header->waktu_entry
                ? \Carbon\Carbon::parse($header->waktu_entry)->format('d/m/Y H:i')
                : null;
        ...
        @endphp
        <x-pdf.identitas-pasien :rm="$header->reg_no ?? null" :nama="$header->reg_name ?? null" :jenisKelamin="$sex === 'L' ? 'Laki-laki' : ($sex === 'P' ? 'Perempuan' : null)" :tempatLahir="$header->birth_place ?? null"
            :tglLahir="$header->birth_date ?? null" :umur="$umur" :alamat="$header->address ?? null">
            <tr>
            ...
            <tr>
                <td class="py-0.5 text-[11px] text-gray-500 whitespace-nowrap">Tgl. Bacaan Radiologi</td>
                <td class="py-0.5 text-[11px] px-1">:</td>
                <td class="py-0.5 text-[11px]">{{ $header->tgl_bacaan ?? '-' }}</td>
            </tr>
    ...
    <style>
        ...
        .rad-hasil-bacaan .ql-align-center {
            text-align: center;
        }

        .rad-hasil-bacaan .ql-align-right {
            text-align: right;
        }

        .rad-hasil-bacaan .ql-align-justify {
            text-align: justify;
        }

        .rad-hasil-bacaan .ql-indent-1 {
            padding-left: 24px;
        }

        .rad-hasil-bacaan .ql-indent-2 {
            padding-left: 48px;
        }

        .rad-hasil-bacaan .ql-indent-3 {
            padding-left: 72px;
        }
    </style>
    ...
    @php
        $drRadiolog = !empty($header->dr_radiologi)
            ? \DB::table('rsmst_doctors')
                ->where('dr_id', $header->dr_radiologi)
                ->first(['dr_id', 'dr_name'])
            : null;
    ...
    <table class="w-full text-[10px] mt-4 border-collapse">
        <tr>
            ...
            <td class="w-1/3 px-1 text-center" style="vertical-align: bottom;">
                <p class="mb-1">Dokter Radiologi,</p>
                @if (!empty($ttdDrRadiolog))
                    <img class="h-16" src="@ttdSrc($ttdDrRadiolog)" alt="">
                @else
                    <div class="h-20"></div>
                @endif
                <div class="inline-block min-w-[130px] pt-0.5">
                    <p class="font-semibold" style="text-decoration: underline;">{{ $drRadiolog->dr_name ?? '-' }}</p>
                </div>
            </td>
        </tr>
    </table>
