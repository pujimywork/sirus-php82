...
new class extends Component {
    ...
    #[On('cetak-rekam-medis-rj-fisio.open')]
    public function open(int $rjNo): mixed
    {
        ...
        $pdf = Pdf::loadView('pages.components.rekam-medis.r-j.cetak-rekam-medis.cetak-rekam-medis-rj-fisio-print', [
            'dataDaftarTxn' => $dataRJ,
            'dataPasien' => ['pasien' => $pasien],
            'namaDokter' => $dokter->dr_name ?? null,
        ])->setPaper('A4');

        $filename = 'fisio-rj-' . ($dataRJ['regNo'] ?? $rjNo) . '-' . ($dataRJ['rjNo'] ?? '') . '.pdf';
