 ...
 * ======================= */
    ...
    /**
     * Flag konteks: true jika component di-mount dari EMR RI.
     * Mengaktifkan tombol "Copy Asesmen UGD → Pengkajian Awal Perawat".
     */
    public bool $contextRI = false;
 ...
 * ======================= */
    ...
    /**
     * Copy Asesmen Perawat dari kunjungan UGD ke Pengkajian Awal RI.
     * Hanya valid kalau component di-mount dengan contextRI=true.
     * Listener di rm-pengkajian-awal-ri-actions akan fetch data UGD & set fields.
     */
    public function copyAssessmentPerawatUGD(int $txnNoUGD): void
    {
        if (!$this->contextRI) {
                                                        ...
                                                        @if ($isRI)
                                                            ...
                                                            <div class="p-2 rounded bg-surface-soft">
                                                                <div class="flex items-center mb-1 space-x-1">
                                                                    ...
                                                                    <span class="text-sm font-semibold">Tindak Lanjut:</span>
                                                                </div>
                                                                <div class="text-sm">
                                                                    @php
                                                                        $tlRI = data_get($datadaftar_json, 'perencanaan.tindakLanjut.tindakLanjut', '');
                                                                        $tglPulangRI = data_get($datadaftar_json, 'perencanaan.tindakLanjut.tglPulang', '');
                                                                    @endphp
                                                                    @if ($tlRI)
                                                            ...
                                                            <div class="grid grid-cols-1 gap-2">
                                                                @if ($isRJ)
                                                                    <div class="grid grid-cols-1 gap-2">
                                                                        ...
                                                                        @hasanyrole('Perawat|Admin|Casemix|Mr|Manager Medis|Manager Umum')
                                                                            <x-outline-button type="button"
                                                                                wire:click="OpenRekamMedisRjV1('{{ $myQData->txn_no }}')"
                                                                                wire:loading.attr="disabled"
                                                                                wire:target="OpenRekamMedisRjV1('{{ $myQData->txn_no }}')">
                                                                                <span wire:loading.remove
                                                                                    wire:target="OpenRekamMedisRjV1('{{ $myQData->txn_no }}')"
                                                                                    class="flex items-center gap-1">
                                                                                    <svg class="w-4 h-4" fill="none"
