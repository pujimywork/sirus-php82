...
new class extends Component {
    ...
    public string $filterKategori = self::KATEGORI_ALL;

    public function mount(): void
    {
        $current = Carbon::now()->year;
        $this->tahunMulai = $current - 4;
        $this->tahunAkhir = $current;
    }
    ...
    {
        $awal = min($this->tahunMulai, $this->tahunAkhir);
        $akhir = max($this->tahunMulai, $this->tahunAkhir);
        $start = Carbon::create($awal, 1, 1)->startOfYear();
        $end = Carbon::create($akhir, 12, 31)->endOfYear();
    ...
    {
        [$start, $end, $awal, $akhir] = $this->periodeRange();
        $aggregate = $this->buildTrfAggregate($start, $end, "TO_CHAR(h.trf_date,'YYYY')", $this->filterKategori);

        $result = [];
        for ($y = $awal; $y <= $akhir; $y++) {
            $key = (string) $y;
            $result[] = $this->fillTrfRow($aggregate->get($key), $key, $key);
        }
        return $result;
    }
...
<div>
    @php
        $tot = $this->totals;
        [$_s, $_e, $awalT, $akhirT] = $this->periodeRange();
        $chartKey = md5("trf-tahunan-{$awalT}-{$akhirT}-{$filterKategori}");
    @endphp
        ...
        <div x-cloak x-show="open"
            ...
            <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
                <div class="p-3 bg-brand-green/5 border border-brand-green/30 rounded-xl dark:border-brand-lime/30 dark:bg-brand-lime/5">
                    <div class="text-xs text-brand-green uppercase dark:text-brand-lime">Mulai</div>
                    <x-text-input type="number" wire:model.live.debounce.500ms="tahunMulai"
                        min="2000" max="2099" maxlength="4"
                        class="mt-1 block w-full !text-xl !font-bold !py-0.5" />
                </div>
                <div class="p-3 bg-brand-green/5 border border-brand-green/30 rounded-xl dark:border-brand-lime/30 dark:bg-brand-lime/5">
                    <div class="text-xs text-brand-green uppercase dark:text-brand-lime">Akhir</div>
                    <x-text-input type="number" wire:model.live.debounce.500ms="tahunAkhir"
                        min="2000" max="2099" maxlength="4"
                        class="mt-1 block w-full !text-xl !font-bold !py-0.5" />
                </div>
