...
new class extends Component {
    ...
    public function rows(): array
    {
        ...
        $aggregate = $this->buildPermintaanLabAggregate(
            $start, $end,
            "to_char(h.rj_date, 'YYYY')",
            "to_char(h.exit_date, 'YYYY')"
        );
...
<div>
    @php
        ...
        $chartKey = md5("lab-tahunan-{$tahunFrom}-{$tahunTo}");
    @endphp
        ...
        <button type="button" @click="open = !open"
            ...
            <div class="flex-1 min-w-0">
                <div class="text-sm font-semibold text-gray-700 dark:text-gray-200">
                    Ringkasan Permintaan Lab {{ $tahunMin }}&ndash;{{ $tahunMax }}
        ...
        <div x-cloak x-show="open" class="px-4 pb-4 border-t border-gray-200 dark:border-gray-700"
            ...
            <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
                <div class="p-3 bg-brand-green/5 border border-brand-green/30 rounded-xl dark:border-brand-lime/30 dark:bg-brand-lime/5">
                    ...
                    <div class="mt-0.5 text-[10px] text-gray-500 dark:text-gray-400 truncate" title="{{ $tahunMin }}–{{ $tahunMax }} ({{ $tahunMax - $tahunMin + 1 }} tahun)">{{ $tahunMin }}&ndash;{{ $tahunMax }} ({{ $tahunMax - $tahunMin + 1 }} thn)</div>
                </div>
    ...
    <div class="mt-4 bg-white border border-gray-200 shadow-sm rounded-2xl dark:border-gray-700 dark:bg-gray-900">
        <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
            <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200">
                ...
                <span class="ml-2 font-normal text-xs text-gray-500">(berdasarkan dokter penanggung jawab kasus, periode terpilih)</span>
            </h3>
        </div>
