...
new class extends Component {
    ...
    public function rows(): array
    {
        ...
        $aggregate = $this->buildPermintaanRadAggregate($start, $end, "to_char(h.rj_date, 'MM')", "to_char(h.exit_date, 'MM')");
    ...
    #[Computed] public function totals(): array { return $this->totalsPermintaan($this->rows); }
    #[Computed] public function chartData(): array { return $this->chartDataPermintaan($this->rows); }
    #[Computed] public function topDokter() { [$start, $end] = $this->periodeRange(); return $this->topDokterRad($start, $end, 10); }
};
?>
...
<div>
    @php
        $tot = $this->totals;
        $chartKey = md5("rad-bulanan-{$filterTahun}");
    @endphp
    ...
    <div class="mt-4 bg-white border border-gray-200 rounded-2xl dark:border-gray-700 dark:bg-gray-900" x-data="{ open: false }">
        <button type="button" @click="open = !open"
            ...
            <div class="flex-1 min-w-0">
                <div class="text-sm font-semibold text-gray-700 dark:text-gray-200">Ringkasan Permintaan Radiologi {{ $filterTahun }}</div>
        ...
        <div x-cloak x-show="open" class="px-4 pb-4 border-t border-gray-200 dark:border-gray-700"
            ...
            <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
                <div class="p-3 bg-brand-green/5 border border-brand-green/30 rounded-xl dark:border-brand-lime/30 dark:bg-brand-lime/5">
                    ...
                    <div class="mt-0.5 text-[10px] text-gray-500 dark:text-gray-400 truncate">Januari&ndash;Desember {{ $filterTahun }}</div>
                </div>
                <div class="p-3 bg-white border border-gray-200 rounded-xl dark:border-gray-700 dark:bg-gray-900">
                    <div class="text-xs text-gray-500 uppercase">Total Permintaan</div>
                ...
                <tbody>
                    @foreach ($this->rows as $r)
                        <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 {{ $r['total'] === 0 ? 'opacity-50' : '' }}">
                            ...
                            <td class="px-3 py-2.5 text-right tabular-nums text-gray-600 dark:text-gray-400">{{ number_format($r['bpjs']) }}</td>
                            <td class="px-3 py-2.5 text-right tabular-nums text-gray-600 dark:text-gray-400">{{ number_format($r['umum']) }}</td>
    ...
    <div class="mt-4 bg-white border border-gray-200 shadow-sm rounded-2xl dark:border-gray-700 dark:bg-gray-900">
        <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
            <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200">
                Top 10 Dokter Pengirim Radiologi
                <span class="ml-2 font-normal text-xs text-gray-500">(dari kolom dr_pengirim, periode terpilih)</span>
            </h3>
        </div>
                ...
                <tbody>
                    @forelse ($this->topDokter as $i => $dr)
                        ...
                        <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td class="px-3 py-2.5 font-medium text-gray-800 dark:text-gray-100">{{ $dr->dr_name ?? '(Tanpa Dokter / ' . ($dr->dr_id ?? '-') . ')' }}</td>
