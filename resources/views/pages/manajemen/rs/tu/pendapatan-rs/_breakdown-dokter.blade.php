...
<div class="mt-6 space-y-4">
        ...
        <div x-show="open" x-cloak class="overflow-auto border-t border-emerald-200 dark:border-emerald-800">
            <table class="w-full text-xs text-left text-gray-700 dark:text-gray-300 table-auto">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="6" class="px-3 py-3 text-center text-gray-500 italic">Tidak ada data RJ pada periode ini.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    {{-- UGD — per dokter --}}
    <div class="bg-white border border-rose-200 rounded-2xl dark:border-rose-800 dark:bg-gray-900"
         x-data="{ open: true }">
        <button type="button" @click="open = !open"
            ...
            <table class="w-full text-xs text-left text-gray-700 dark:text-gray-300 table-auto">
                ...
                <tbody>
                    @forelse ($dokterUgd as $row)
                        <tr class="border-t border-gray-200 dark:border-gray-700 hover:bg-rose-50/50 dark:hover:bg-rose-900/10">
                            <td class="px-3 py-1.5">{{ $row['dr_name'] }} <span class="text-gray-400 text-[10px]">({{ $row['dr_id'] }})</span></td>
                            <td class="px-3 py-1.5 text-right font-mono">{{ number_format($row['jumlah'], 0, ',', '.') }}</td>
                            <td class="px-3 py-1.5 text-right font-mono">{{ number_format($row['bpjs'], 0, ',', '.') }}</td>
                            <td class="px-3 py-1.5 text-right font-mono text-gray-500">{{ number_format($row['umum'], 0, ',', '.') }}</td>
                    ...
                    @empty
                        <tr><td colspan="5" class="px-3 py-3 text-center text-gray-500 italic">Tidak ada data UGD pada periode ini.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="bg-white border border-blue-200 rounded-2xl dark:border-blue-800 dark:bg-gray-900"
         x-data="{ open: true }">
        <button type="button" @click="open = !open"
            ...
            <table class="w-full text-xs text-left text-gray-700 dark:text-gray-300 table-auto">
                ...
                <tbody>
                    @forelse ($dokterRi as $row)
                        <tr class="border-t border-gray-200 dark:border-gray-700 hover:bg-blue-50/50 dark:hover:bg-blue-900/10
                            @if ($row['dr_id'] === '__NONE__') bg-amber-50/40 dark:bg-amber-900/10 @endif">
                            <td class="px-3 py-1.5">
                                {{ $row['dr_name'] }}
                                @if ($row['dr_id'] !== '__NONE__')
                                    <span class="text-gray-400 text-[10px]">({{ $row['dr_id'] }})</span>
                                @endif
                            </td>
                            <td class="px-3 py-1.5 text-right font-mono">{{ number_format($row['jumlah'], 0, ',', '.') }}</td>
                            <td class="px-3 py-1.5 text-right font-mono">{{ number_format($row['bpjs'], 0, ',', '.') }}</td>
                    ...
                    @empty
                        <tr><td colspan="5" class="px-3 py-3 text-center text-gray-500 italic">Tidak ada data RI pada periode ini.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
