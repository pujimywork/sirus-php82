...
new class extends Component {
    ...
    public function rows(): array
    {
        ...
        $aggregate = $this->buildKunjunganUGDAggregate($start, $end, "to_char(h.rj_date, 'MM')");
...
<div>
    @php
        $tot = $this->totals;
        $chartKey = md5("ugd-bulanan-{$filterTahun}");
    @endphp
        ...
        <button type="button" @click="open = !open"
            ...
            <div class="flex-1 min-w-0">
                <div class="text-sm font-semibold text-gray-700 dark:text-gray-200">
                    Ringkasan Kunjungan UGD {{ $filterTahun }}
                </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 dark:bg-gray-800">
                    <tr class="text-xs font-semibold tracking-wide text-gray-600 uppercase dark:text-gray-300">
                        ...
                        <th class="px-2 py-3 text-right text-gray-600 dark:text-gray-400" title="Triase P4 — Non-darurat">P4</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($this->rows as $r)
                        <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 {{ $r['total'] === 0 ? 'opacity-50' : '' }}">
                            ...
                            <td class="px-2 py-2.5 text-right tabular-nums text-gray-600 dark:text-gray-400">{{ number_format($r['p4']) }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot class="bg-gray-100 dark:bg-gray-800 border-t-2 border-gray-300 dark:border-gray-600">
                    <tr class="text-sm font-bold text-gray-800 dark:text-gray-100">
                        ...
                        <td class="px-2 py-3 text-right tabular-nums text-gray-700 dark:text-gray-300">{{ number_format($tot['p4']) }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
    ...
    @php
        ...
        $triaseRows = [
            ...
            ['code' => 'P4', 'label' => 'Non-darurat',     'count' => $tot['p4'], 'cls' => 'bg-gray-400 dark:bg-gray-500',       'text' => 'text-gray-700 dark:text-gray-300'],
        ];
    @endphp
    <div class="mt-4 bg-white border border-gray-200 shadow-sm rounded-2xl dark:border-gray-700 dark:bg-gray-900">
        <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                ...
                <tbody>
                    @foreach ($triaseRows as $tr)
                        ...
                        <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td class="px-3 py-2.5 font-medium text-gray-800 dark:text-gray-100">{{ $tr['label'] }}</td>
                    ...
                    @if ($tot['triase_kosong'] > 0)
                        ...
                        <tr class="border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30">
                            <td class="px-4 py-2.5">
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium text-gray-500">
                                    &mdash;
                                </span>
                            </td>
                            <td class="px-3 py-2.5 italic text-gray-500 dark:text-gray-400">Tanpa Triase</td>
                            <td class="px-3 py-2.5 text-right tabular-nums text-gray-500">{{ number_format($tot['triase_kosong']) }}</td>
                            <td class="px-3 py-2.5">
                                <div class="flex items-center gap-2">
                                    <div class="flex-1 h-2 bg-gray-200 rounded-full dark:bg-gray-700">
                                        <div class="h-2 rounded-full bg-gray-300 dark:bg-gray-600" style="width: {{ min(100, round($pctKosong, 1)) }}%"></div>
                                    </div>
                                    <span class="text-xs text-gray-600 dark:text-gray-400 tabular-nums w-12 text-right">{{ round($pctKosong, 1) }}%</span>
                                </div>
                            </td>
                        </tr>
                    @endif
    ...
    <div class="mt-4 bg-white border border-gray-200 shadow-sm rounded-2xl dark:border-gray-700 dark:bg-gray-900">
        <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
            <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200">
                ...
                <span class="ml-2 font-normal text-xs text-gray-500">{{ count($this->dokterBreakdown) }} dokter</span>
            </h3>
        </div>
                ...
                <tbody>
                    @forelse ($this->dokterBreakdown as $i => $dokter)
                        ...
                        <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td class="px-3 py-2.5 font-medium text-gray-800 dark:text-gray-100">{{ $dokter->dr_name ?? '(Tanpa Dokter)' }}</td>
