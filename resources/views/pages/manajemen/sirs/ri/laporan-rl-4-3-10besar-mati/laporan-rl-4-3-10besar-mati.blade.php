...
use Carbon\Carbon;
use App\Http\Traits\Manajemen\Sirs\Ri\RL43Trait;
...
new class extends Component {
    ...
    {
        return $this->computeRL43($this->tahun);
    }
        ...
        <div class="w-full px-4 py-2 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold leading-tight text-gray-900 dark:text-gray-100">
                    Laporan RL 4.3 &mdash; 10 Besar Kematian Penyakit Rawat Inap
                </h2>
                <p class="text-base text-gray-700 dark:text-gray-700">
                    Top 10 ICD-10 dengan jumlah pasien meninggal terbanyak di Rawat Inap per tahun.
                    <span class="text-gray-400">Sorted desc by jumlah meninggal (BUKAN total H+M seperti RL 4.2).
                    ICD tanpa kematian dikecualikan.</span>
                </p>
            </div>
            ...
            <div class="bg-white border border-gray-200 shadow-sm rounded-2xl dark:border-gray-700 dark:bg-gray-900">
                <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200">
                        RL 4.3 &mdash; Top 10 ICD-10 Penyebab Kematian di Rawat Inap
                        <span class="ml-2 font-normal text-xs text-gray-500">
                            ({{ count($this->rows) }} ICD top mati, total {{ number_format($tot['mati_total']) }} pasien meninggal)
                        </span>
                    </h3>
                </div>
                    ...
                    <table class="min-w-full text-sm border-collapse">
                        <thead class="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200">
                            <tr class="text-xs font-semibold tracking-wider uppercase">
                                ...
                                <th colspan="3" class="px-2 py-3 text-center border border-gray-200 dark:border-gray-700 text-rose-700 dark:text-rose-300">Pasien Keluar Mati (sort key)</th>
                            </tr>
                            <tr class="text-xs font-semibold tracking-wider uppercase">
                        ...
                        <tbody>
                            @forelse ($this->rows as $i => $r)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                                    ...
                                    <td class="px-2 py-2 border-r border-gray-200 dark:border-gray-700 text-right tabular-nums text-emerald-700 dark:text-emerald-300">{{ number_format($r['total']) }}</td>
                                    ...
                                    <td class="px-2 py-2 text-right tabular-nums text-rose-700 dark:text-rose-300 font-bold text-base">{{ number_format($r['mati_total']) }}</td>
                                </tr>
                            @empty
                                <tr><td colspan="9" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400 italic">Tidak ada pasien RI meninggal di tahun {{ $tahun }}</td></tr>
                            @endforelse
                        </tbody>
                ...
                <div class="px-4 py-2 text-[10px] text-gray-500 dark:text-gray-500 border-t border-gray-100 dark:border-gray-800 leading-relaxed">
                    ...
                    <strong>Sorting:</strong> desc by jumlah pasien meninggal (mati_total). Beda dari RL 4.2 yang sort by total H+M.
                    <strong>"Mati"</strong>: SNOMED 419099009 di JSON.
                    <strong>Exclude:</strong> ICD tanpa kematian (mati_total = 0) tidak masuk ranking. Pasien tanpa diagnosis utama di-skip.
