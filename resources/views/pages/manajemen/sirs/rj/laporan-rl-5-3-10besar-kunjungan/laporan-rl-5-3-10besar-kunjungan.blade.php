...
use Carbon\Carbon;
use App\Http\Traits\Manajemen\RL53Trait;
        ...
        <div class="w-full px-4 py-2 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold leading-tight text-gray-900 dark:text-gray-100">
                    Laporan RL 5.3 &mdash; 10 Besar Kunjungan Penyakit Rawat Jalan
                </h2>
                <p class="text-base text-gray-700 dark:text-gray-700">
                    Top 10 ICD-10 dengan jumlah kunjungan terbanyak di RJ poliklinik per tahun.
                    <span class="text-gray-400">Sorted desc by total kunjungan (L + P).
                    "Kasus Baru" = pasien unik (DISTINCT reg_no), "Kunjungan" = total visits.</span>
                </p>
            </div>
                    ...
                    <table class="min-w-full text-sm border-collapse">
                        <thead class="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200">
                            <tr class="text-xs font-semibold tracking-wider uppercase">
                                ...
                                <th colspan="3" class="px-2 py-3 text-center border border-gray-200 dark:border-gray-700 text-emerald-700 dark:text-emerald-300">Jumlah Kasus Baru</th>
                                <th colspan="3" class="px-2 py-3 text-center border border-gray-200 dark:border-gray-700 text-purple-700 dark:text-purple-300">Jumlah Kunjungan (sort key)</th>
                            </tr>
                            <tr class="text-xs font-semibold tracking-wider uppercase">
                                <th class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right text-blue-700 dark:text-blue-300">L</th>
                                <th class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right text-pink-700 dark:text-pink-300">P</th>
                                ...
                                <th class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right text-blue-700 dark:text-blue-300">L</th>
                                <th class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right text-pink-700 dark:text-pink-300">P</th>
                                <th class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right text-purple-700 dark:text-purple-300">Total</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($this->rows as $i => $r)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                                    ...
                                    <td class="px-2 py-2 border-r border-gray-200 dark:border-gray-700 text-right tabular-nums text-blue-700 dark:text-blue-300">{{ number_format($r['kasus_l']) }}</td>
                                    <td class="px-2 py-2 border-r border-gray-200 dark:border-gray-700 text-right tabular-nums text-pink-700 dark:text-pink-300">{{ number_format($r['kasus_p']) }}</td>
                                    <td class="px-2 py-2 border-r border-gray-200 dark:border-gray-700 text-right tabular-nums text-emerald-700 dark:text-emerald-300 font-semibold">{{ number_format($r['kasus_total']) }}</td>
                                    <td class="px-2 py-2 border-r border-gray-200 dark:border-gray-700 text-right tabular-nums text-blue-700 dark:text-blue-300">{{ number_format($r['kunj_l']) }}</td>
                                    <td class="px-2 py-2 border-r border-gray-200 dark:border-gray-700 text-right tabular-nums text-pink-700 dark:text-pink-300">{{ number_format($r['kunj_p']) }}</td>
                            ...
                            @empty
                                <tr><td colspan="9" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400 italic">Belum ada data RJ di tahun {{ $tahun }}</td></tr>
                            @endforelse
                        </tbody>
                        @if (count($this->rows) > 0)
                            <tfoot class="bg-gray-100 dark:bg-gray-800 border-t-2 border-gray-300 dark:border-gray-600 font-bold">
                                <tr class="text-sm text-gray-800 dark:text-gray-100">
                                    ...
                                    <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right tabular-nums text-blue-800 dark:text-blue-200">{{ number_format($tot['kasus_l']) }}</td>
                                    <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right tabular-nums text-pink-800 dark:text-pink-200">{{ number_format($tot['kasus_p']) }}</td>
                                    <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right tabular-nums text-emerald-800 dark:text-emerald-200">{{ number_format($tot['kasus_total']) }}</td>
                                    <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right tabular-nums text-blue-800 dark:text-blue-200">{{ number_format($tot['kunj_l']) }}</td>
                                    <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right tabular-nums text-pink-800 dark:text-pink-200">{{ number_format($tot['kunj_p']) }}</td>
                                    <td class="px-2 py-3 border border-gray-200 dark:border-gray-700 text-right tabular-nums text-purple-800 dark:text-purple-200">{{ number_format($tot['kunj_total']) }}</td>
                                </tr>
                            </tfoot>
                        @endif
                    </table>
