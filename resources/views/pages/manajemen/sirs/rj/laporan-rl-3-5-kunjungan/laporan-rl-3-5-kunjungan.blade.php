...
use Carbon\Carbon;
use App\Http\Traits\Manajemen\RL35Trait;
                    ...
                    <table class="min-w-full text-xs border-collapse">
                        <thead class="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200">
                            <tr class="text-[10px] font-semibold tracking-wider uppercase">
                                ...
                                <th rowspan="2" class="px-3 py-2 border border-gray-200 dark:border-gray-700 text-left sticky left-12 bg-gray-100 dark:bg-gray-800 z-20 min-w-[16rem]">Jenis Kegiatan</th>
                                <th colspan="2" class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-center text-emerald-700 dark:text-emerald-300">Kunjungan Pasien Dalam Kota</th>
                                <th colspan="2" class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-center text-amber-700 dark:text-amber-300">Kunjungan Pasien Luar Kota</th>
                                <th rowspan="2" class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right">Total Kunjungan</th>
                            </tr>
                            <tr class="text-[10px] font-semibold tracking-wider uppercase">
                                <th class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right text-emerald-700 dark:text-emerald-300">Laki-Laki</th>
                                <th class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right text-emerald-700 dark:text-emerald-300">Perempuan</th>
                                <th class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right text-amber-700 dark:text-amber-300">Laki-Laki</th>
                                <th class="px-2 py-2 border border-gray-200 dark:border-gray-700 text-right text-amber-700 dark:text-amber-300">Perempuan</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($this->rows as $r)
                                ...
                                @endphp
                                <tr class="{{ $rowCls }} border-b border-gray-100 dark:border-gray-800">
                                    <td class="px-2 py-1.5 border-r border-gray-200 dark:border-gray-700 text-center font-mono text-gray-500 sticky left-0 z-10 {{ $isSpecial ? 'bg-inherit' : 'bg-white dark:bg-gray-900' }}">{{ $r['no'] }}</td>
                                    <td class="px-3 py-1.5 border-r border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-100 sticky left-12 z-10 {{ $isSpecial ? 'bg-inherit' : 'bg-white dark:bg-gray-900' }}">{{ $r['nama'] }}</td>
                                    <td class="px-2 py-1.5 border-r border-gray-200 dark:border-gray-700 text-right tabular-nums text-emerald-700 dark:text-emerald-300">{{ number_format($r['dalam_l']) }}</td>
                                    <td class="px-2 py-1.5 border-r border-gray-200 dark:border-gray-700 text-right tabular-nums text-emerald-700 dark:text-emerald-300">{{ number_format($r['dalam_p']) }}</td>
                                    <td class="px-2 py-1.5 border-r border-gray-200 dark:border-gray-700 text-right tabular-nums text-amber-700 dark:text-amber-300">{{ number_format($r['luar_l']) }}</td>
                                    <td class="px-2 py-1.5 border-r border-gray-200 dark:border-gray-700 text-right tabular-nums text-amber-700 dark:text-amber-300">{{ number_format($r['luar_p']) }}</td>
