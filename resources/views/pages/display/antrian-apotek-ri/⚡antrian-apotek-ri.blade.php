    ...
    <div class="w-full min-h-screen px-4 pt-4 pb-2"
        ...
        <div class="h-[calc(100vh-220px)] overflow-auto"
                            ...
                            @forelse ($rowsAntri as $row)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-3 py-2.5 truncate">
                                        <span class="text-xs text-gray-500">{{ $row->bangsal_name ?: '-' }}{{ $row->room_name ? ' · ' . $row->room_name : '' }}</span><br>
                                        {{ $row->dr_name ?: '-' }}
                                    </td>
                                    <td class="px-2 py-2.5 text-center">
                                        <div class="text-2xl font-bold text-gray-900 leading-none">{{ $row->no_antrian ?: '—' }}</div>
                                        <div class="mt-1">
                                            @if ($row->has_resep)
            ...
            @if ($rowsAntri->count() === 0 && $rowsLunas->count() === 0)
                <div class="w-full p-6 text-center text-sm text-gray-500">Belum ada antrian resep RI hari ini.</div>
            @endif
        </div>
