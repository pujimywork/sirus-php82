    ...
    @if (!empty($pasienData))
        ...
        <div class="grid grid-cols-5 gap-3">
            ...
            <div class="col-span-3">
                <div class="h-full px-4 py-3 space-y-1 text-sm border rounded-lg bg-surface-soft dark:bg-gray-800/50">
                    {{-- Nama + JK --}}
                    <div class="flex items-center gap-2">
                        <div
                            class="flex items-center justify-center w-8 h-8 text-sm font-bold rounded-full shrink-0
                            {{ $p['sex'] === 'L' ? 'bg-blue-100 text-blue-700' : ($p['sex'] === 'P' ? 'bg-pink-100 text-pink-700' : 'bg-surface-soft text-muted') }}">
                            {{ $p['sex'] === 'L' ? 'L' : ($p['sex'] === 'P' ? 'P' : '-') }}
                        </div>
                        <div>
                            <div class="text-lg font-semibold text-ink dark:text-white">
                                {{ $p['regName'] }}
                            </div>
                    ...
                    <div class="flex items-center gap-4">
                        <div>
                            ...
                            <span class="ml-1 font-mono font-medium text-ink dark:text-gray-200">{{ $p['regNo'] }}</span>
                        </div>
                        <div>
                            <span class="text-muted">Tgl Lahir:</span>
                            <span class="ml-1 text-body dark:text-gray-300">{{ $p['birthDate'] }}
                                ({{ $p['umur'] }})</span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <span class="ml-1 text-body dark:text-gray-300">{{ $p['address'] }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-2">
                <div class="h-full px-4 py-3 space-y-2 text-sm border rounded-lg bg-surface-soft dark:bg-gray-800/50">
                    ...
                    <div class="flex items-center justify-between gap-2">
                        <div>
                            ...
                            <span class="ml-1 text-body dark:text-gray-300">{{ $p['checkupDate'] }}</span>
                        </div>
                    </div>
                    ...
                    @if ($p['refNo'] !== '-' && !empty($p['refNo']))
                        <div>
                            ...
                            <span class="ml-1 font-mono text-body dark:text-gray-300">{{ $p['refNo'] }}</span>
                        </div>
                    @endif
                    ...
                    <div>
                        ...
                        @if (!empty($p['empName']))
                            <span class="ml-1 text-body dark:text-gray-300">{{ $p['empName'] }}</span>
                        @else
