...
new class extends Component {
    ...
    {
        $this->dispatch('penerimaan-non-medis.openCreate');
    }

    public function openEdit(string $rcvNo): void
    {
        $this->dispatch('penerimaan-non-medis.openEdit', rcvNo: $rcvNo);
    }

    public function requestDelete(string $rcvNo): void
    {
        $this->dispatch('penerimaan-non-medis.requestDelete', rcvNo: $rcvNo);
    }

    public function requestBatal(string $rcvNo): void
    {
        $this->dispatch('penerimaan-non-medis.requestBatal', rcvNo: $rcvNo);
    }

    #[On('penerimaan-non-medis.saved')]
    ...
    {
        $query = DB::table('imtxn_receivehdrsnon as a')
            ...
            ->select([
                ...
                DB::raw("(SELECT SUM(
                    ...
                    - NVL(d.dtl_diskon1,0)
                ) FROM imtxn_receivedtlsnon d WHERE d.rcv_no = a.rcv_no) as total_detail"),
                'a.rcv_diskon',
                'a.rcv_ppn',
                'a.rcv_ppn_status',
                'a.rcv_materai',
...
<div>
    ...
    <div class="w-full min-h-[calc(100vh-5rem)] bg-white dark:bg-gray-800">
        <div class="px-6 pt-2 pb-6">
                ...
                <div class="flex flex-wrap items-end gap-3">
                    ...
                    <div class="flex items-end gap-2 ml-auto">
                        ...
                        <x-primary-button type="button" wire:click="openCreate" class="whitespace-nowrap"
                            ...
                            Tambah Barang Non-Medis dari Supplier
                        </x-primary-button>
                    </div>
                </div>
            </div>
            ...
            <livewire:pages::transaksi.gudang.penerimaan-non-medis.penerimaan-non-medis-actions
                wire:key="penerimaan-non-medis-actions" />
        </div>
    </div>
</div>
