...
new class extends Component {
    ...
    {
        $this->dispatch('transfer-stock-non.openCreate', slCodeFrom: self::WAREHOUSE_SL_CODE);
    }

    public function openEdit(int $trfNo): void
    {
        $this->dispatch('transfer-stock-non.openEdit', trfNo: $trfNo);
    }

    public function requestBatal(int $trfNo): void
    {
        $this->dispatch('transfer-stock-non.requestBatal', trfNo: $trfNo);
    }

    #[On('transfer-stock-non.saved')]
    ...
    {
        $query = DB::table('imtxn_trfhdrnonmedes as h')
            ...
            ->select([
                ...
                DB::raw('(SELECT COUNT(*) FROM imtxn_trfdtlnonmedes d WHERE d.trf_no = h.trf_no) as item_count'),
                DB::raw('(SELECT NVL(SUM(d.qty),0) FROM imtxn_trfdtlnonmedes d WHERE d.trf_no = h.trf_no) as qty_total'),
            ])
...
<div>
    ...
    <div class="w-full h-[calc(100vh-5rem)] flex flex-col bg-canvas dark:bg-gray-800">
        <div class="flex flex-col flex-1 min-h-0 px-6 pt-2 pb-6">
                ...
                <div class="flex flex-wrap items-end gap-3">
                    ...
                    <div class="flex items-end gap-2 ml-auto">
                        ...
                        <x-primary-button type="button" wire:click="openCreate" class="whitespace-nowrap"
                            title="Buat transfer non-medis baru">
                        ...
                        <tbody class="text-body divide-y divide-hairline dark:divide-gray-700 dark:text-gray-200">
                            @forelse($this->rows as $row)
                                @php
                                    ...
                                    $editable = $st === 'A';
                                @endphp
                                <tr wire:key="trfnon-row-{{ $row->trf_no }}"
            ...
            <livewire:pages::transaksi.gudang.transfer-stock-non.transfer-stock-non-actions
                wire:key="transfer-stock-non-actions" />
