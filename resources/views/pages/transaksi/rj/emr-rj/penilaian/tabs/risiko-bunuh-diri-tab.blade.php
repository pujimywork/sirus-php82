{{-- pages/transaksi/rj/emr-rj/penilaian/tabs/risiko-bunuh-diri-tab.blade.php --}}
...
<div class="space-y-4">
                        ...
                        @foreach ($ideBunuhDiriPertanyaan as $key => $pertanyaan)
                            <div class="grid items-center grid-cols-1 gap-2 sm:grid-cols-[1fr_230px]">
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <x-radio-button label="Ya" value="Ya" name="ideBunuhDiriRj{{ $key }}"
                                        wire:model.live="formEntryResikoBunuhDiri.ideBunuhDiri.{{ $key }}" />
                                    <x-radio-button label="Tidak" value="Tidak" name="ideBunuhDiriRj{{ $key }}"
                                        wire:model.live="formEntryResikoBunuhDiri.ideBunuhDiri.{{ $key }}" />
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        @foreach ($perilakuBunuhDiriPertanyaan as $key => $pertanyaan)
                            <div class="grid items-center grid-cols-1 gap-2 sm:grid-cols-[1fr_230px]">
                                <x-input-label :value="$loop->iteration . '. ' . $pertanyaan" />
                                <div class="grid grid-cols-2 gap-2">
                                    <x-radio-button label="Ya" value="Ya" name="perilakuBunuhDiriRj{{ $key }}"
                                        wire:model.live="formEntryResikoBunuhDiri.perilakuBunuhDiri.{{ $key }}" />
                                    <x-radio-button label="Tidak" value="Tidak" name="perilakuBunuhDiriRj{{ $key }}"
                                        wire:model.live="formEntryResikoBunuhDiri.perilakuBunuhDiri.{{ $key }}" />
                                </div>
                            </div>
                        @endforeach
    ...
    @if (collect($dataDaftarPoliRJ['penilaian']['resikoBunuhDiri'] ?? [])->filter(fn($entri) => filled(data_get($entri, 'tglPenilaian')))->isNotEmpty())
            ...
            <div class="overflow-x-auto rounded-lg border border-hairline dark:border-gray-700">
                <table class="w-full text-sm text-left text-muted dark:text-gray-300">
                    ...
                    <tbody class="divide-y divide-hairline-soft dark:divide-gray-700">
                        @foreach (array_reverse(array_filter($dataDaftarPoliRJ['penilaian']['resikoBunuhDiri'] ?? [], fn($entri) => filled(data_get($entri, 'tglPenilaian'))), true) as $i => $row)
                            @php
