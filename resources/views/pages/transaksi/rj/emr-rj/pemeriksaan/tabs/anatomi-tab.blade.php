...
<div class="space-y-4" wire:key="{{ $this->renderKey('modal-pengkajian-dokter-ri', [$riHdrNo ?? 'new']) }}">
    ...
    <x-border-form title="Bagian 2.2 — Pemeriksaan Anatomi" align="start" bgcolor="bg-gray-50">
        ...
        <div class="mt-4" x-data="{ activeTabAnatomi: 'kepala' }">
                ...
                <div class="flex-1 min-w-0">
                    @foreach ($anatomiList as $key => $label)
                        <div x-show="activeTabAnatomi === '{{ $key }}'"
                            ...
                            <div>
                                <x-input-label :value="strtoupper($label)" class="mb-2" />
                                <div class="flex items-center gap-4">
                                    @foreach (['Tidak Diperiksa', 'Tidak Ada Kelainan', 'Ada'] as $opt)
                                        <label class="flex items-center gap-2 cursor-pointer">
                                            <input type="radio"
                                                wire:model.live="dataDaftarRi.pengkajianDokter.anatomi.{{ $key }}.kelainan"
                                                value="{{ $opt }}" {{ $isFormLocked ? 'disabled' : '' }}
                                                class="w-4 h-4 text-brand border-gray-300 focus:ring-brand" />
                                            <span
                                                class="text-sm text-gray-700 dark:text-gray-300">{{ $opt }}</span>
                                        </label>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <x-textarea
                                    ...
                                    placeholder="{{ strtoupper($label) }}" :disabled="$isFormLocked" rows="4"
                                    class="w-full mt-1" />
                            </div>
