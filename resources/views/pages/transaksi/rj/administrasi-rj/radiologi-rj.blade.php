...
new class extends Component {
    ...
    /* Status transaksi induk RJ → lock + reason granular (seragam gaya modul lab).
       'A' aktif; L=pulang, F=batal, I=transfer ke UGD. */
    private function evaluasiLockRJ(): void
    {
        ...
        $s = DB::table('rstxn_rjhdrs')->where('rj_no', $this->rjNo)->value('rj_status');

        $this->formLockReason = match ($s) {
            'A' => '',
            'L' => 'Pasien RJ sudah pulang — data radiologi terkunci.',
            'F' => 'Transaksi RJ sudah dibatalkan — data radiologi terkunci.',
            'I' => 'Transaksi RJ sudah ditransfer ke UGD — data radiologi terkunci.',
            default => empty($s) ? '' : 'Transaksi RJ tidak aktif — data radiologi terkunci.',
        };
