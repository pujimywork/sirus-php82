    ...
    private function findData(int $rjNo): array
    {
        ...
        if ($hdr->pass_status === 'N') {
            if (!isset($data['rjAdmin'])) {
                ...
                DB::table('rstxn_rjhdrs')->where('rj_no', $rjNo)->update(['rj_admin' => $data['rjAdmin']]);
            } else {
        ...
        if (!isset($data['rsAdmin'])) {
            ...
            DB::table('rstxn_rjhdrs')->where('rj_no', $rjNo)->update(['rs_admin' => $data['rsAdmin']]);
        } else {
            $data['rsAdmin'] = (int) ($hdr->rs_admin ?? 0);
        }

        // ── Poli Price (Uang Periksa) ──
        $klaimStatus =
        ...
        if (!isset($data['poliPrice'])) {
            ...
            $data['poliPrice'] = $hdrVal > 0 ? $hdrVal : (int) $dokterPoliPrice;
            DB::table('rstxn_rjhdrs')->where('rj_no', $rjNo)->update(['poli_price' => $data['poliPrice']]);
        } else {
