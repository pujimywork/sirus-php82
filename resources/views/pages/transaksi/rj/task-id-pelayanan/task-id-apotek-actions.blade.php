...
/**
 * Gabungan aksi Task ID apotek RJ (TaskId6 Masuk Apotek, TaskId7 Keluar Apotek)
 * dalam SATU komponen per baris.
 *
 * Sebelumnya 2 komponen Livewire terpisah (task-id-6, task-id-7) di-mount per baris
 * di antrian-apotek-rj → 2x jumlah komponen (pemicu payload berat / TooManyComponents).
 * Digabung: logika tiap aksi identik (guard, prasyarat, penomoran noAntrianApotek
 * anti-race di dalam lock, push antrian farmasi + taskId ke BPJS), spinner per tombol
 * tetap terisolasi via wire:target. Root display:contents + wrapper inline-block per
 * tombol → tata letak grid parent tidak berubah. Hanya dipakai di antrian-apotek-rj.
 */
...
<div class="contents">
    ...
    <div class="inline-block">
        <x-success-button wire:click="prosesTaskId6" wire:loading.attr="disabled" wire:target="prosesTaskId6"
