<?php
// resources/views/pages/transaksi/ri/emr-ri/modul-dokumen/penundaan-pelayanan-ri/rm-penundaan-pelayanan-ri-actions.blade.php
...
new class extends Component {
    ...
    public function mount(?string $riHdrNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-penundaan-pelayanan-ri']);
    ...
    public function openModal(): void
    {
        ...
        $this->dispatch('open-modal', name: "rm-penundaan-pelayanan-ri-{$this->riHdrNo}");
    }
    ...
    {
        $this->dispatch('close-modal', name: "rm-penundaan-pelayanan-ri-{$this->riHdrNo}");
    }
    ...
    public function addEntry(): void
    {
        ...
        try {
            DB::transaction(function () use ($entry) {
                ...
                if (!isset($fresh['penundaanPelayananRI']) || !is_array($fresh['penundaanPelayananRI'])) {
                    $fresh['penundaanPelayananRI'] = [];
                }

                $fresh['penundaanPelayananRI'][] = $entry;

                $this->updateJsonRI((int) $this->riHdrNo, $fresh);
                $this->dataDaftarRi = $fresh;
                $this->penundaanList = $fresh['penundaanPelayananRI'];

                $this->appendAdminLogRI((int) $this->riHdrNo, 'Tambah Pemberitahuan Penundaan/Kelambatan — TTD ' . ($entry['signatureDate'] ?? '-'), 'MR');
            });
    ...
    public function cetak(string $signatureDate)
    {
        ...
        try {
            ...
            // TTD Pemberi Informasi (myuser_code → myuser_ttd_image)
            $ttdPemberiPath = null;
            ...
            $pdf = Pdf::loadView('pages.components.modul-dokumen.r-i.penundaan-pelayanan-ri.cetak-penundaan-pelayanan-ri-print', ['data' => $data])->setPaper('A4');

            $this->dispatch('toast', type: 'success', message: 'Berhasil mencetak formulir penundaan.');
            return response()->streamDownload(fn() => print $pdf->output(), 'penundaan-pelayanan-ri-' . ($pasien['regNo'] ?? $this->riHdrNo) . '.pdf');
        } catch (\Throwable $e) {
    ...
    public function hapus(string $signatureDate): void
    {
        ...
        try {
            DB::transaction(function () use ($signatureDate) {
                ...
                $fresh = $this->findDataRI($this->riHdrNo) ?: [];
                if (!isset($fresh['penundaanPelayananRI'])) {
                    throw new \RuntimeException('Data formulir tidak ditemukan.');
                }

                $fresh['penundaanPelayananRI'] = collect($fresh['penundaanPelayananRI'])
            ...
            <div class="flex-1 px-4 py-4 bg-surface-soft/70 dark:bg-gray-950/20">
                <div class="max-w-full mx-auto space-y-4">
                    ...
                    <livewire:pages::transaksi.ri.display-pasien-ri.display-pasien-ri :riHdrNo="$riHdrNo"
                        wire:key="pp-ri-display-pasien-{{ $riHdrNo ?? 'init' }}" />
                            ...
                            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <x-signature.ttd-petugas :framed="false" :allowClear="false"
                                        :ttd="$newForm['pemberiInfo']" :date="$newForm['pemberiInfoDate'] ?? ''"
                                        :code="$newForm['pemberiInfoCode'] ?? ''" :locked="$isFormLocked"
                                        sign="setPemberiInfo" label="" signLabel="TTD sebagai Pemberi Informasi" />
                                </div>
                            </div>
                        </section>
