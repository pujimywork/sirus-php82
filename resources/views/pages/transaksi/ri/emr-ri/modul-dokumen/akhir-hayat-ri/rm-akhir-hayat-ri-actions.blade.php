...
new class extends Component {
    ...
    /**
     * Warna chip skala simptom — hijau → kuning → oranye → merah, supaya tingkat
     * keparahan langsung terbaca tanpa mengeja teksnya. 'aktif' = terpilih (solid),
     * 'diam' = belum terpilih (tint tipis).
     */
    public array $warnaSkala = [
        'tidakAda' => [
            'aktif' => 'bg-emerald-600 border-emerald-600 text-white shadow-sm',
            'diam' => 'bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100 dark:bg-emerald-900/20 dark:border-emerald-800/40 dark:text-emerald-300',
        ],
        'ringan' => [
            'aktif' => 'bg-yellow-500 border-yellow-500 text-white shadow-sm',
            'diam' => 'bg-yellow-50 border-yellow-200 text-yellow-800 hover:bg-yellow-100 dark:bg-yellow-900/20 dark:border-yellow-800/40 dark:text-yellow-300',
        ],
        'sedang' => [
            'aktif' => 'bg-orange-500 border-orange-500 text-white shadow-sm',
            'diam' => 'bg-orange-50 border-orange-200 text-orange-800 hover:bg-orange-100 dark:bg-orange-900/20 dark:border-orange-800/40 dark:text-orange-300',
        ],
        'berat' => [
            'aktif' => 'bg-red-600 border-red-600 text-white shadow-sm',
            'diam' => 'bg-red-50 border-red-200 text-red-700 hover:bg-red-100 dark:bg-red-900/20 dark:border-red-800/40 dark:text-red-300',
        ],
    ];
    ...
    private function defaultForm(): array
    {
        return [
            ...
            'psikososial' => [
                ...
                'orangDihubungi' => ['ada' => '', 'nama' => '', 'hubungan' => '', 'alamat' => '', 'telp' => ''],
            ],
            'spiritual' => [
                'perluPelayanan' => '',
                'oleh' => '',
            ...
            'alternatif' => ['pilihan' => '', 'donasiOrgan' => '', 'lainnya' => ''],
    ...
    /** Isi awal dari data EMR yang sudah ada (diagnosis & TTV terakhir) — tetap bisa dikoreksi. */
    private function prefillDariEmr(): void
    {
        ...
        $tandaVital = data_get($this->dataDaftarRi, 'pengkajianAwalPasienRawatInap.bagian4PemeriksaanFisik.tandaVital', []);
        foreach ([
            'tb' => 'tb',
            'bb' => 'bb',
            'sistolik' => 'sistolik',
            'distolik' => 'distolik',
            'nadi' => 'frekuensiNadi',
            'respirasi' => 'frekuensiNafas',
            'suhu' => 'suhu',
            'spo2' => 'spo2',
        ] as $tujuan => $asal) {
            if (empty($this->form['fisik'][$tujuan]) && filled(data_get($tandaVital, $asal))) {
                $this->form['fisik'][$tujuan] = (string) data_get($tandaVital, $asal);
            }
        }
    }
    ...
    private function akhirHayatRules(): array
    {
        $rules = [
            ...
            'form.ttd.saksiNama' => 'nullable|string|max:250',
            ...
            'saksiSignature' => 'nullable|string',
        ];
                            ...
                            <div class="mt-3 space-y-3">
                                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                                    <div>
                                        ...
                                        <x-text-input wire:model.blur="form.medis.diagnosaUtama" class="w-full mt-1"
                                            placeholder="Diagnosis utama" :disabled="$formRO" />
                                    </div>
                                    <div>
                                        <x-input-label value="Diagnosis Sekunder" />
                                        <x-text-input wire:model.blur="form.medis.diagnosaSekunder" class="w-full mt-1"
                                            placeholder="Diagnosis penyerta" :disabled="$formRO" />
                                    </div>
                                </div>

                                <div class="grid grid-cols-1 gap-3 md:grid-cols-2 items-start">
                                    <div>
                                ...
                                <div class="grid grid-cols-3 gap-2 sm:grid-cols-5 lg:grid-cols-9 items-end">
                                    @foreach ([['tb', 'TB (cm)'], ['bb', 'BB (kg)'], ['sistolik', 'Sistolik'], ['distolik', 'Diastolik'], ['nadi', 'Nadi'], ['respirasi', 'Napas'], ['suhu', 'Suhu (°C)'], ['spo2', 'Saturasi O₂ (%)']] as [$fieldFisik, $labelFisik])
                                        <div wire:key="fisik-{{ $fieldFisik }}">
                                            ...
                                            <x-text-input wire:model.blur="form.fisik.{{ $fieldFisik }}" type="number" step="any"
                                                class="w-full mt-1" :disabled="$formRO" />
                                        </div>
                                    @endforeach
                                    <div>
                                        <x-input-label value="IMT" />
                                        <div class="px-2 py-2 mt-1 text-sm font-semibold text-center border rounded-lg border-hairline bg-surface-soft text-ink dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100">
                                            {{ $this->bmi ?? '—' }}
                                        </div>
                                    </div>
                                </div>
                                <p class="text-xs text-muted dark:text-gray-400">
                                    Diagnosis &amp; tanda vital terisi otomatis dari Pengkajian Awal bila sudah ada — koreksi bila berubah.
                                    IMT dihitung otomatis dari TB &amp; BB.
                                </p>
                            </div>
                        </fieldset>
                    </x-border-form>
                            ...
                            <div class="mt-3 space-y-4">
                                <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4 items-start">
                                    @foreach ([['nyeri', 'Nyeri'], ['sesak', 'Sesak napas'], ['mualMuntah', 'Mual / muntah'], ['kelelahan', 'Kelelahan']] as [$fieldSimptom, $labelSimptom])
                                        @php
                                            $nilaiSimptom = $form['simptom'][$fieldSimptom] ?? '';
                                            // Bingkai kartu ikut warna tingkat keparahan yang terpilih
                                            $bingkaiKartu = match ($nilaiSimptom) {
                                                'tidakAda' => 'border-emerald-300 dark:border-emerald-800/60',
                                                'ringan' => 'border-yellow-300 dark:border-yellow-800/60',
                                                'sedang' => 'border-orange-300 dark:border-orange-800/60',
                                                'berat' => 'border-red-300 dark:border-red-800/60',
                                                default => 'border-hairline dark:border-gray-700',
                                            };
                                        @endphp
                                        <div class="p-3 border rounded-lg bg-canvas dark:bg-gray-800 {{ $bingkaiKartu }}"
                                            wire:key="simptom-{{ $fieldSimptom }}">
                                            <x-input-label :value="$labelSimptom" />
                                            <div class="grid grid-cols-2 gap-1.5 mt-1.5">
                                                @foreach ($skalaSimptom as $val => $labelSkala)
                                                    <button type="button" wire:key="skala-{{ $fieldSimptom }}-{{ $val }}"
                                                        wire:click="$set('form.simptom.{{ $fieldSimptom }}', '{{ $nilaiSimptom === $val ? '' : $val }}')"
                                                        @disabled($formRO)
                                                        class="px-2 py-1.5 text-xs font-semibold text-center border rounded-lg transition-colors
                                                               disabled:opacity-60 disabled:cursor-not-allowed
                                                               {{ $nilaiSimptom === $val ? $warnaSkala[$val]['aktif'] : $warnaSkala[$val]['diam'] }}">
                                                        {{ $labelSkala }}
                                                    </button>
                                                @endforeach
                                            </div>
                                ...
                                @if (($form['simptom']['nyeri'] ?? '') !== '' && ($form['simptom']['nyeri'] ?? '') !== 'tidakAda')
                                    <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                                        <div>
                                            <x-input-label value="Lokasi nyeri" />
                                            <x-text-input wire:model.blur="form.simptom.nyeriLokasi" class="w-full mt-1" :disabled="$formRO" />
                                        </div>
                                        <div>
                                            <x-input-label value="Deskripsi nyeri" />
                                            <x-text-input wire:model.blur="form.simptom.nyeriDeskripsi" class="w-full mt-1"
                                ...
                                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 items-start">
                                    ...
                                    <div class="p-3 border rounded-xl border-hairline bg-canvas dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <x-textarea wire:model.blur="form.psikososial.emosional" class="w-full mt-1" rows="2"
                                            placeholder="Uraian singkat kondisi emosional pasien" :disabled="$formRO" />
                                    ...
                                    <div class="p-3 border rounded-xl border-hairline bg-canvas dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <x-textarea wire:model.blur="form.psikososial.emosionalKeluarga" class="w-full mt-1" rows="2"
                                            placeholder="Uraian singkat kondisi emosional keluarga" :disabled="$formRO" />
                                    ...
                                    @if (($form['psikososial']['orangDihubungi']['ada'] ?? '') === 'ya')
                                        <div class="grid grid-cols-1 gap-3 mt-2 md:grid-cols-4">
                                            <div>
                                                ...
                                                <x-text-input wire:model.blur="form.psikososial.orangDihubungi.nama" class="w-full mt-1" :disabled="$formRO" />
                                            </div>
                                            <div>
                                                <x-input-label value="Hubungan" />
                                                <x-text-input wire:model.blur="form.psikososial.orangDihubungi.hubungan" class="w-full mt-1" :disabled="$formRO" />
                                            </div>
                                            <div>
                                                <x-input-label value="Alamat" />
                                                <x-text-input wire:model.blur="form.psikososial.orangDihubungi.alamat" class="w-full mt-1" :disabled="$formRO" />
                                            </div>
                                            <div>
                                    ...
                                    <div class="p-3 border rounded-lg border-hairline bg-surface-soft/60 dark:bg-gray-900/40 dark:border-gray-700">
                                        ...
                                        <div class="flex flex-wrap gap-2 mt-1">
                                            @foreach (['tidak' => 'Tidak ada', 'autopsi' => 'Autopsi', 'donasi' => 'Donasi organ', 'lainnya' => 'Lainnya'] as $val => $label)
                                                <x-radio-button :label="$label" :value="$val" name="alternatifPelayanan"
                                                    wire:model.live="form.alternatif.pilihan" :disabled="$formRO" />
                                            @endforeach
                                        </div>
                    ...
                    <x-border-form title="5. Pernyataan Persetujuan & Tanda Tangan" align="start"
                        ...
                        <fieldset @disabled($formRO)>
                            ...
                            <div class="grid grid-cols-1 gap-4 mt-4 sm:grid-cols-2 lg:grid-cols-3 items-stretch">
                                ...
                                <div class="flex flex-col h-full p-3 border rounded-lg border-hairline bg-surface-soft/60 dark:bg-gray-900/40 dark:border-gray-700">
                                    <p class="mb-2 text-xs font-semibold tracking-wide text-center uppercase text-muted dark:text-gray-400">
                                        Saksi <span class="font-normal normal-case">(opsional)</span>
                                    </p>
                                    <div class="flex-1">
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <x-text-input wire:model.blur="form.ttd.saksiNama" class="w-full mt-1"
                                            placeholder="Kosongkan bila tanpa saksi"
