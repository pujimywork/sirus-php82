...
new class extends Component {
    ...
    // ── Form entri baru (Instruksi Pasca Bedah — PAB 7.3 / RM 56) ──
    ...
    public function mount(?string $riHdrNo = null, bool $disabled = false): void
    {
        ...
        if ($this->riHdrNo) {
            ...
            if ($data) {
                ...
                $this->instruksiList = $data['instruksiPascaBedahRI'] ?? [];
    ...
    public function openModal(): void
    {
        ...
        $this->dataDaftarRi = $data;
        $this->regNo = $data['regNo'] ?? null;
        if (!isset($this->dataDaftarRi['instruksiPascaBedahRI']) || !is_array($this->dataDaftarRi['instruksiPascaBedahRI'])) {
            $this->dataDaftarRi['instruksiPascaBedahRI'] = [];
        }
        $this->instruksiList = $this->dataDaftarRi['instruksiPascaBedahRI'];
    ...
    public function addEntry(): void
    {
        ...
        try {
            DB::transaction(function () use ($entry) {
                ...
                if (!isset($fresh['instruksiPascaBedahRI']) || !is_array($fresh['instruksiPascaBedahRI'])) {
                    $fresh['instruksiPascaBedahRI'] = [];
                }

                $fresh['instruksiPascaBedahRI'][] = $entry;

                $this->updateJsonRI((int) $this->riHdrNo, $fresh);
                $this->dataDaftarRi = $fresh;
                $this->instruksiList = $fresh['instruksiPascaBedahRI'];

                $this->appendAdminLogRI((int) $this->riHdrNo, 'Tambah Instruksi Pasca Bedah — ' . ($entry['createdAt'] ?? '-'), 'MR');
            });

            $this->incrementVersion('modal-instruksi-pasca-bedah-ri');
            $this->dispatch('toast', type: 'success', message: 'Instruksi pasca bedah berhasil disimpan.');
    ...
    public function hapus(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $fresh = $this->findDataRI($this->riHdrNo) ?: [];
                if (!isset($fresh['instruksiPascaBedahRI'])) {
                    throw new \RuntimeException('Data instruksi tidak ditemukan.');
                }

                $fresh['instruksiPascaBedahRI'] = collect($fresh['instruksiPascaBedahRI'])
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex-1 space-y-3">
                ...
                <p class="text-base text-muted dark:text-gray-400">
                    ...
                    minum, infus, monitor tanda vital. Diisi & ditandatangani ahli anestesiologi.
                </p>
                @if ($ipbCount > 0)
                    <ul class="space-y-1 text-base text-muted dark:text-gray-300 list-disc pl-5">
                        @foreach (array_slice($instruksiList, 0, 3) as $ipb)
                            <li>
                                <span class="font-medium">{{ \Illuminate\Support\Str::limit($ipb['bilaKesakitan'] ?? '-', 50) ?: '-' }}</span>
                                @if (!empty($ipb['tanggal']))
                                    <span class="text-sm text-muted-soft">— {{ $ipb['tanggal'] }}</span>
                                @endif
                            </li>
                        @endforeach
                        @if ($ipbCount > 3)
                            <li class="text-sm italic text-muted-soft">+{{ $ipbCount - 3 }} lainnya…</li>
                        @endif
                    </ul>
                @endif
            </div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="mt-0.5 text-base text-muted dark:text-gray-400">
                                    RM 56 — PAB 7.3 · diisi & ditandatangani ahli anestesiologi
                                </p>
                            </div>
                        </div>
                    ...
                    <div
                        ...
                        <section class="pt-6 space-y-4 border-t border-hairline dark:border-gray-700">
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <div>
                                    ...
                                    <x-textarea wire:model.live="newForm.bilaKesakitan" :error="$errors->has('newForm.bilaKesakitan')" rows="2"
                                        :disabled="$isFormLocked" class="w-full" />
                                </div>
                                <div>
                                    <x-input-label value="Bila Mual / Muntah" class="mb-1" />
                                    <x-textarea wire:model.live="newForm.bilaMualMuntah" :error="$errors->has('newForm.bilaMualMuntah')" rows="2"
                                        :disabled="$isFormLocked" class="w-full" />
                                </div>
                                <div>
                                    <x-input-label value="Antibiotik" class="mb-1" />
                                    <x-textarea wire:model.live="newForm.antibiotik" :error="$errors->has('newForm.antibiotik')" rows="2"
                                        :disabled="$isFormLocked" class="w-full" />
                                </div>
                                <div>
                                    <x-input-label value="Obat-obatan Lain" class="mb-1" />
                                    <x-textarea wire:model.live="newForm.obatLain" :error="$errors->has('newForm.obatLain')" rows="2" :disabled="$isFormLocked"
                                        class="w-full" />
                                </div>
                                <div>
                                    <x-input-label value="Minum" class="mb-1" />
                                    <x-text-input wire:model.live="newForm.minum" :error="$errors->has('newForm.minum')"
                                        placeholder="cth: Boleh minum bertahap bila sadar penuh" :disabled="$isFormLocked"
                                        class="w-full" />
                                </div>
                                <div>
                                    <x-input-label value="Infus" class="mb-1" />
                                    <x-text-input wire:model.live="newForm.infus" :error="$errors->has('newForm.infus')" placeholder="cth: RL 20 tpm"
                                        :disabled="$isFormLocked" class="w-full" />
                                </div>
                            </div>
                        </section>
                        ...
                        <section class="pt-6 space-y-3 border-t border-hairline dark:border-gray-700">
                            ...
                            <div class="flex flex-wrap gap-4">
                                <x-toggle wire:model.live="newForm.monitorTensi" :trueValue="true" :falseValue="false"
                                    label="Tensi" :disabled="$isFormLocked" />
                                <x-toggle wire:model.live="newForm.monitorNadi" :trueValue="true" :falseValue="false"
                                    label="Nadi" :disabled="$isFormLocked" />
                                <x-toggle wire:model.live="newForm.monitorNafas" :trueValue="true" :falseValue="false"
                                    label="Nafas" :disabled="$isFormLocked" />
                            </div>
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <div>
                                    <x-input-label value="Setiap" class="mb-1" />
                                    <x-text-input wire:model.live="newForm.monitorSetiap" :error="$errors->has('newForm.monitorSetiap')" placeholder="cth: 10 menit"
                                        :disabled="$isFormLocked" class="w-full" />
                                </div>
                                <div>
                                    <x-input-label value="Selama" class="mb-1" />
                                    <x-text-input wire:model.live="newForm.monitorSelama" :error="$errors->has('newForm.monitorSelama')" placeholder="cth: 2 jam"
                                        :disabled="$isFormLocked" class="w-full" />
                                </div>
                            </div>
                        </section>
                        ...
                        <section class="pt-6 border-t border-hairline dark:border-gray-700">
                            ...
                            <x-textarea wire:model.live="newForm.lainLain" :error="$errors->has('newForm.lainLain')" rows="3" :disabled="$isFormLocked"
                                class="w-full" />
                        </section>

                        {{-- ══ TTD AHLI ANESTESIOLOGI ══ --}}
                        <section class="pt-6 space-y-4 border-t border-hairline dark:border-gray-700">
                            <h3 class="text-base font-semibold text-ink dark:text-gray-200">Tanda Tangan Ahli
                                Anestesiologi</h3>
                            <div class="max-w-md">
                                        ...
                                        <div
                                            ...
                                            <x-primary-button wire:click.prevent="setTtd" wire:loading.attr="disabled"
                                                ...
                                                <span wire:loading.remove wire:target="setTtd"
                                                    ...
                                                    TTD sebagai Ahli Anestesiologi
                                                </span>
                                ...
                                <table
                                    ...
                                    <thead class="bg-surface-soft dark:bg-gray-800">
                                        <tr class="text-left text-muted dark:text-gray-300">
                                            ...
                                            <th class="px-4 py-2 border-b">Ringkasan</th>
                                            <th class="px-4 py-2 border-b">TTD</th>
                                            <th class="px-4 py-2 border-b text-center">Aksi</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($instruksiList as $ipb)
                                            <tr
                                                ...
                                                <td class="px-4 py-2 font-medium text-ink dark:text-gray-200">
                                                    {{ $ipb['bilaKesakitan'] ? Str::limit($ipb['bilaKesakitan'], 45) : '-' }}
                                                </td>
                                                <td class="px-4 py-2 text-muted dark:text-gray-400">
                                                    {{ $ipb['ttd'] ?? '-' }}</td>
                                                <td class="px-4 py-2 text-center space-x-2 whitespace-nowrap">
                                                    <x-secondary-button wire:click="cetak('{{ $ipb['createdAt'] }}')"
                                                        wire:loading.attr="disabled"
                                                        wire:target="cetak('{{ $ipb['createdAt'] }}')"
                                                    ...
                                                    @if (!$isFormLocked)
                                                        <x-outline-button type="button"
                                                            wire:click.prevent="hapus('{{ $ipb['createdAt'] }}')"
                                                            wire:confirm="Yakin hapus instruksi ini?"
                                                            wire:loading.attr="disabled"
                ...
                <div class="flex flex-wrap items-center justify-end gap-3">
                    ...
                    @if ($riHdrNo && !$isFormLocked)
                        <x-primary-button wire:click.prevent="addEntry" wire:loading.attr="disabled"
                            ...
                            <span wire:loading.remove wire:target="addEntry">Simpan Instruksi</span>
