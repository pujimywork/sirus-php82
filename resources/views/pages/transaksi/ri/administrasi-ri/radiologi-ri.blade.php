            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse ($dataRad as $item)
                        <tr wire:key="radiologi-ri-{{ $item['rirad_no'] ?? $loop->index }}" class="transition hover:bg-gray-50 dark:hover:bg-gray-800/40">
                            <td class="px-4 py-3 font-mono text-xs text-gray-500 whitespace-nowrap">{{ $item['rirad_date'] ?? '-' }}</td>
                            <td class="px-4 py-3 font-mono text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">{{ $item['rad_id'] }}</td>
                            <td class="px-4 py-3 text-gray-800 dark:text-gray-200 whitespace-nowrap">{{ $item['rad_desc'] }}</td>
