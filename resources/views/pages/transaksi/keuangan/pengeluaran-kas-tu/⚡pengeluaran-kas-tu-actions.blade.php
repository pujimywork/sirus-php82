...
new class extends Component {
    ...
    public function openCreate(): void
    {
        ...
        $this->dispatch('focus-tucashk-co-date');
    }
    ...
    public function openEdit(string $tucashkNo): void
    {
        ...
        $this->incrementVersion('modal');
        $this->dispatch('open-modal', name: 'pengeluaran-kas-tu-actions');
        $this->dispatch('focus-tucashk-co-desc');
    }

    /* ── LOV Listeners ── */
    #[On('lov.selected.akun-co-tu')]
    public function onAkunCOSelected(string $target, ?array $payload): void
    {
        ...
        $this->dispatch('focus-lov-kas-co');
    }

    #[On('lov.selected.akun-kas-tu-co')]
        ...
        try {
            DB::transaction(function () use ($empId, $shift) {
                if ($this->editNo) {
                    ...
                    $nextNo = DB::selectOne("SELECT tucashd_seq.NEXTVAL AS val FROM dual")->val;
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                                    Catat pengeluaran kas (Cash-Out) di luar transaksi pelayanan RS.
                                </p>
                            </div>
                        </div>
            ...
            <div class="flex-1 px-4 py-4 bg-gray-50/70 dark:bg-gray-950/20">
                <div class="max-w-4xl">
                    <x-border-form title="Data Pengeluaran Kas (Cash-Out)"
                        x-data
                        x-on:focus-tucashk-co-date.window="$nextTick(() => setTimeout(() => $refs.inputTucashkCoDate?.focus(), 150))"
                        x-on:focus-tucashk-co-desc.window="$nextTick(() => setTimeout(() => $refs.inputTucashkCoDesc?.focus(), 150))"
                        x-on:focus-lov-kas-co.window="$nextTick(() => setTimeout(() => $refs.lovKasCoWrapper?.querySelector('input:not([disabled])')?.focus(), 150))"
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                ...
                                <x-text-input type="text" wire:model="tucashkDate" placeholder="dd/mm/yyyy hh:mm:ss"
                                    x-ref="inputTucashkCoDate"
                                    x-on:keydown.enter.prevent="$refs.inputTucashkCoDesc?.focus()"
                                    class="w-full mt-1" />
                            ...
                            <div>
                                ...
                                <x-text-input type="text" wire:model="tucashkDesc" placeholder="Keterangan pengeluaran kas"
                                    x-ref="inputTucashkCoDesc"
                                    x-on:keydown.enter.prevent="$refs.inputTucashkCoNominal?.focus()"
                                    class="w-full mt-1" />
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                {{-- Akun Pengeluaran (CO) --}}
                                <div>
                                    <livewire:lov.akun-co.lov-akun-co target="akun-co-tu" label="Akun Pengeluaran (CO)" :initialAccId="$accId"
                                        wire:key="lov-co-{{ $editNo ?? 'new' }}-{{ $renderVersions['modal'] ?? 0 }}" />
                                    <x-input-error :messages="$errors->get('accId')" class="mt-1" />
                                </div>

                                {{-- Akun Kas --}}
                                <div x-ref="lovKasCoWrapper">
                                    <livewire:lov.kas.lov-kas target="akun-kas-tu-co" tipe="" label="Akun Kas" :initialAccId="$accIdKas"
