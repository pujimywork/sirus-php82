...
/**
 * Topup Supplier Non-Medis (ATK, alat tulis, dll).
 *
 * Equivalent dgn Oracle Forms procedure post_transaksi_topup (versi non-medis):
 *   - Input: supplier (filter nonmedis) + akun kas + tanggal + nominal + keterangan
 *   - INSERT 1x IMTXN_CASHOUTHDRTOPUPSNON (saldo titipan, tidak link ke nota)
 *
 * Beda dgn pembayaran-hutang-non-medis (angsuran):
 *   - Topup tidak link ke rcv_no spesifik (cuma cashouthdrtopupsnon)
 *   - Tidak ada cashoutdtls / receivepayments
 *
 * Sumber tabel:
 *   - IMTXN_CASHOUTHDRTOPUPSNON  (sirus, non-medis)
 *   - IMMST_SUPPLIERS            (filter nonmedis='Y')
...
new class extends Component {
    ...
    /* ── LOV Listeners ── */
    #[On('lov.selected.topup-non-medis-supplier')]
    ...
    #[On('lov.selected.topup-non-medis-kas')]
    ...
    public function history()
    {
        ...
        return DB::table('imtxn_cashouthdrtopupsnon')
            ->select([
                'cashout_no',
    ...
    public function totalTopup(): int
    {
        ...
        return (int) DB::table('imtxn_cashouthdrtopupsnon')
            ->where('supp_id', $this->suppId)
            ->sum('cashout_value');
    }
    ...
    public function simpan(): void
    {
        ...
        try {
            DB::transaction(function () use ($empId, $shift, $tanggalDb, $desc) {
                ...
                DB::table('imtxn_cashouthdrtopupsnon')->insert([
...
<div>
    <x-page-title
        title="Topup Supplier Non-Medis"
        subtitle="Setor DP/uang muka ke supplier non-medis — ATK, alat tulis, dll (saldo titipan)" />
        ...
        <div class="px-6 pt-4 pb-6 space-y-4">
            ...
            <div class="bg-canvas border border-hairline shadow-sm rounded-2xl dark:border-gray-700 dark:bg-gray-900">
                <div class="flex items-center justify-between px-5 py-4 border-b border-hairline dark:border-gray-700">
                    <div>
                        <h3 class="text-base font-semibold text-ink dark:text-gray-100">1. Pilih Supplier (Non-Medis)</h3>
                ...
                <div class="px-5 py-5">
                    <livewire:lov.supplier.lov-supplier
                        target="topup-non-medis-supplier"
                        label="Cari Supplier"
                        placeholder="Ketik kode / nama / telp supplier..."
                        jenisSupplier="nonmedis"
                        :initialSuppId="$suppId"
                        wire:key="lov-supplier-topup-non-medis-{{ $suppId ?? 'empty' }}" />
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 gap-5 px-5 py-5 sm:grid-cols-2">
                        ...
                        <div>
                            <livewire:lov.kas.lov-kas
                                target="topup-non-medis-kas"
                                tipe=""
                                label="Akun Kas"
                                placeholder="Ketik kode/nama kas..."
                                :initialAccId="$accId"
                                wire:key="lov-kas-topup-non-medis-{{ $suppId }}-{{ $renderVersions['form'] ?? 0 }}" />
                        ...
                        <div>
                            ...
                            <x-text-input type="text" wire:model="keterangan"
                                placeholder="Misal: DP order ATK batch 2026-05" class="w-full mt-1" />
                        </div>
                    </div>
                        ...
                        <table class="min-w-full text-sm">
                            ...
                            <tbody class="text-body divide-y divide-hairline dark:divide-gray-700 dark:text-gray-200">
                                @forelse($this->history as $row)
                                    <tr wire:key="topup-non-medis-{{ $row->cashout_no ?? $loop->index }}" class="hover:bg-surface-soft dark:hover:bg-gray-800/60">
