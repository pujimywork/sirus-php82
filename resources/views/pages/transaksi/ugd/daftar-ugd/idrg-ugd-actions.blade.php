<?php
// Komponen Modal Bridging iDRG / INACBG (E-Klaim Kemenkes) UGD.
// Dipisah dari daftar-ugd-actions supaya orchestrator daftar-ugd-actions tetap ramping.
// Trigger dari parent: dispatch event 'daftar-ugd.idrg.open' dengan rjNo.
...
new class extends Component {
     ...
     * SFC dispatch `idrg-section-changed-ugd` setiap kali saveResult / state berubah.
     ...
     * cross-sibling broadcast (`idrg-state-updated-ugd`), jadi tidak ada race
     * "A request already contains one of the messages" di Livewire bundle interceptor.
     */
    #[On('idrg-section-changed-ugd')]
    ...
    <x-modal name="ugd-idrg" size="full" height="full" focusable>
        @php
            ...
            $guide = [
                ['key' => 'A', 'title' => 'A. Setup Klaim', 'items' => [
                    ...
                    ['n' => 3, 'head' => 'Simpan Data Klaim', 'body' => 'tarif_rs + tanggal masuk/pulang auto dari rincian kasir UGD.'],
                ]],
        ...
        <div class="flex flex-col min-h-0" wire:key="{{ $this->renderKey('modal', [$rjNo ?? 'none']) }}">
            <div class="relative px-6 py-5 border-b border-hairline dark:border-gray-700">
                ...
                {{-- Identitas pasien — pakai komponen standar display-pasien-ugd --}}
                <div class="mt-4">
                    <livewire:pages::transaksi.ugd.display-pasien-ugd.display-pasien-ugd :rjNo="$rjNo"
                        wire:key="{{ $this->renderKey('modal', ['idrg-display-pasien-ugd', $rjNo ?? 'none']) }}" />
                </div>
            </div>
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="flex justify-end px-4 py-3 bg-canvas border border-hairline shadow-sm rounded-xl dark:bg-gray-900 dark:border-gray-700">
                                        <x-primary-button type="button" :disabled="$inacbgFinal"
                                            x-on:click="
                                                Livewire.dispatch('idrg-diagnosa-inacbg-ugd.set', { rjNo: '{{ $rjNo }}' });
                                                Livewire.dispatch('idrg-prosedur-inacbg-ugd.set', { rjNo: '{{ $rjNo }}' });
                                            "
            ...
            <div class="sticky bottom-0 z-10 px-6 py-4 bg-surface-soft border-t border-hairline dark:bg-gray-900 dark:border-gray-700">
                <div class="flex justify-end gap-3">
                    <x-secondary-button x-on:click="$dispatch('close-modal', { name: 'ugd-idrg' })">
                        Tutup
                    </x-secondary-button>
                </div>
            </div>
