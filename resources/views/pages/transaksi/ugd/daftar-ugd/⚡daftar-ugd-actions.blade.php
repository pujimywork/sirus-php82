...
new class extends Component {
    ...
    public function openIdrg(string $rjNo): void
    {
        ...
        $this->incrementVersion('modal');
        $this->dispatch('open-modal', name: 'ugd-idrg');
    }

    public function kirimIdrg(string $step): void
    {
        if (empty($this->rjNo)) {
            $this->dispatch('toast', type: 'error', message: 'Silakan pilih data UGD terlebih dahulu.');
            return;
        }

        $eventMap = [
            'generate-number'     => 'idrg-claim-ugd.generate-number',
            'new-claim'           => 'idrg-claim-ugd.new',
            'set-data'            => 'idrg-claim-ugd.set-data',
            'delete-claim'        => 'idrg-claim-ugd.delete',
            'set-diagnosa-idrg'   => 'idrg-diagnosa-ugd.set',
            'set-prosedur-idrg'   => 'idrg-prosedur-ugd.set',
            'group-idrg'          => 'idrg-grouping-ugd.group',
            'final-idrg'          => 'idrg-grouping-ugd.final',
            'reedit-idrg'         => 'idrg-grouping-ugd.reedit',
            'import-inacbg'       => 'idrg-grouping-ugd.import-inacbg',
            'set-diagnosa-inacbg' => 'idrg-inacbg-ugd.set-diagnosa',
            'set-prosedur-inacbg' => 'idrg-inacbg-ugd.set-prosedur',
            'group-inacbg-1'      => 'idrg-inacbg-ugd.group-stage1',
            'group-inacbg-2'      => 'idrg-inacbg-ugd.group-stage2',
            'final-inacbg'        => 'idrg-inacbg-ugd.final',
            'reedit-inacbg'       => 'idrg-inacbg-ugd.reedit',
            'final-klaim'         => 'idrg-klaim-ugd.final',
            'reedit-klaim'        => 'idrg-klaim-ugd.reedit',
            'send-klaim'          => 'idrg-klaim-ugd.send',
            'print-klaim'         => 'idrg-klaim-ugd.print',
            'get-status'          => 'idrg-klaim-ugd.get-status',
        ];
...
<div>
    ...
    {{-- iDRG (E-Klaim) Modal --}}
    <x-modal name="ugd-idrg" size="full" height="full" focusable>
        @php
            $idrgData = $dataDaftarUGD['idrg'] ?? [];
            ...
            $guide = [
                ['key' => 'A', 'title' => 'A. Setup Klaim', 'items' => [
                    ...
                    ['n' => 2, 'head' => 'Buat Klaim Baru (new_claim)', 'body' => 'Registrasi SEP ke E-Klaim.'],
                    ['n' => 3, 'head' => 'Simpan Data Klaim (set_claim_data)', 'body' => 'tarif_rs + tanggal masuk/pulang auto dari rincian kasir UGD.'],
                ]],
                ['key' => 'B', 'title' => 'B. Coding iDRG', 'items' => [
                    ['n' => 4, 'head' => 'Set Diagnosa iDRG', 'body' => 'Auto dari EMR (Primary di depan).'],
                ...
                ['key' => 'C', 'title' => 'C. Coding INACBG (setelah iDRG final)', 'items' => [
                    ...
                    ['n' => 10, 'head' => 'Grouping INACBG Stage 1', 'body' => 'Hasil kode CBG.'],
                    ['n' => 11, 'head' => 'Grouping Stage 2', 'body' => 'Hanya jika muncul special_cmg_option (implant/prosthesis).'],
                ...
                ['key' => '!', 'title' => '⚠ Penting — UGD Rajal Saja', 'items' => [
                    ['n' => '—', 'head' => 'Scope Klaim UGD', 'body' => 'iDRG di sini khusus UGD yang pasien pulang (jenis_rawat = 2 / rajal). Kalau pasien lanjut rawat inap, klaim dibuat dari modul RI.'],
                    ['n' => '—', 'head' => 'Setelah iDRG Final', 'body' => 'Form EMR (diagnosa/prosedur) harus read-only. Pakai Edit Ulang iDRG kalau perlu revisi.'],
                    ['n' => '—', 'head' => 'Setelah Klaim Final', 'body' => 'Data di-lock penuh sampai Edit Ulang Klaim.'],
                ]],
            ];
        @endphp
            ...
            <div class="relative px-6 py-5 border-b border-gray-200 dark:border-gray-700">
                <div class="relative flex items-start justify-between gap-4">
                    ...
                    <x-icon-button color="gray" type="button"
                        x-on:click="$dispatch('close-modal', { name: 'ugd-idrg' })">
                    ...
                    @if (!empty($this->idrgPdfBase64))
                        <div class="p-4 bg-white border-2 border-brand/40 shadow-sm rounded-xl dark:bg-gray-900 dark:border-brand-lime/40">
                            <div class="flex items-center justify-between mb-3">
                                <h3 class="text-sm font-semibold text-brand dark:text-brand-lime">
                                    PDF Klaim — SEP {{ $this->idrgPdfSep ?? '-' }}</h3>
                                <a href="data:application/pdf;base64,{{ $this->idrgPdfBase64 }}"
                                    download="klaim-{{ $this->idrgPdfSep ?? 'eklaim' }}.pdf"
    ...
    {{-- iDRG Components --}}
    <livewire:pages::transaksi.ugd.idrg.kirim-claim wire:key="idrg-claim-ugd" />
    <livewire:pages::transaksi.ugd.idrg.kirim-diagnosa-idrg wire:key="idrg-diagnosa-ugd" />
    <livewire:pages::transaksi.ugd.idrg.kirim-prosedur-idrg wire:key="idrg-prosedur-ugd" />
    <livewire:pages::transaksi.ugd.idrg.kirim-grouping-idrg wire:key="idrg-grouping-ugd" />
    <livewire:pages::transaksi.ugd.idrg.kirim-grouping-inacbg wire:key="idrg-inacbg-ugd" />
    <livewire:pages::transaksi.ugd.idrg.kirim-final-klaim wire:key="idrg-klaim-ugd" />
</div>
