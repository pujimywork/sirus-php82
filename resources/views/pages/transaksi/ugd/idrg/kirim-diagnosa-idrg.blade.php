...
new class extends Component {
     ...
     *
     * Setelah insert, list disort: Primary di atas.
     */
     ...
     *  1. Kode harus accpdx='Y' — tidak, tolak dengan toast.
     *  2. Single-Primary invariant: Primary lain di-demote ke Secondary.
...
<div class="p-4 space-y-3 bg-canvas border border-hairline shadow-sm rounded-xl dark:bg-gray-900 dark:border-gray-700">
    ...
    <div class="flex items-start justify-between gap-3">
        <div class="flex items-center gap-3">
            <div
                class="flex items-center justify-center w-8 h-8 rounded-full {{ !empty($idrgDiagnosaString) ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-surface-soft text-muted-soft dark:bg-gray-800 dark:text-gray-500' }}">
                <span class="text-sm font-bold">4</span>
            </div>
            <div>
                <div class="font-semibold text-ink dark:text-gray-100">Set Diagnosa iDRG</div>
                <div class="text-sm text-muted dark:text-gray-400">Coder edit di sini, EMR tidak terganggu.</div>
            </div>
        </div>
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-hairline-soft dark:divide-gray-700">
                    @foreach ($coderDiagnosa as $i => $d)
                        <tr wire:key="coder-diag-{{ $i }}-{{ $d['code'] ?? 'x' }}" class="bg-canvas dark:bg-gray-900">
                            ...
                            <td class="px-2 py-1.5 text-center align-top">
                                ...
                                @elseif ($vc === '0')
                                    <div class="inline-flex flex-col items-start gap-0.5 text-left"
                                        ...
                                        @if (!empty($extraPairs))
                                            <ul class="text-[10px] text-muted dark:text-gray-400 leading-tight space-y-0.5 max-w-[220px]">
    ...
    @if (!empty($idrgDiagnosaString))
        <div class="px-2 py-1.5 text-sm font-mono text-muted bg-surface-soft rounded dark:bg-gray-800 dark:text-gray-400">
            <span class="text-muted">Terkirim:</span> {{ $idrgDiagnosaString }}
        </div>
    @endif
    ...
    @if (!empty($idrgDiagnosaExpanded))
        <details class="px-2 py-1 text-sm border border-hairline rounded dark:border-gray-700">
            ...
            <pre class="p-2 mt-1 overflow-x-auto text-[10px] leading-tight bg-surface-soft rounded dark:bg-gray-900">{{ json_encode($idrgDiagnosaExpanded, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) }}</pre>
        </details>
    @endif
</div>
