<?php
// resources/views/pages/transaksi/ugd/administrasi-ugd/jasa-karyawan-ugd.blade.php
...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-jasa-karyawan-ugd'];

    public bool $isFormLocked = false;
    public ?int $rjNo = null;

    /** Daftar jasa karyawan dari line table (gabungan sirus-php82 + Oradev 6i). */
    public array $ugdJasaKaryawan = [];
     ...
     * yang sudah populate kedua field dari rsview_ugdkasir.
     */
    ...
    {
        $this->ugdJasaKaryawan = DB::table('rstxn_ugdactemps as rja')
    ...
    /* ===============================
     ...
     =============================== */
    #[On('administrasi-jasa-karyawan-ugd.updated')]
    ...
    public function insertJasaKaryawan(): void
    {
        ...
        try {
            DB::transaction(function () {
                ...
                $lastInserted = DB::table('rstxn_ugdactemps')->select(DB::raw('nvl(max(acte_dtl)+1,1) as acte_dtl_max'))->first();
    ...
    public function removeJasaKaryawan(int $rjActeDtl): void
    {
        ...
        $itemDesc = collect($this->ugdJasaKaryawan)->firstWhere('rjActeDtl', $rjActeDtl)['JasaKaryawanDesc'] ?? '-';
        ...
        try {
            DB::transaction(function () use ($rjActeDtl, $itemDesc) {
                ...
                DB::table('rstxn_ugdactemps')->where('acte_dtl', $rjActeDtl)->delete();

                $this->appendAdminLogUGD($this->rjNo, 'Hapus Jasa Karyawan: ' . $itemDesc . ' #' . $rjActeDtl);
            });
    ...
    private function insertLainLain(int $rjNo, int $acteDtl, string $otherId, $otherPrice): void
    {
        $validator = Validator::make(
            ['LainLainId' => $otherId, 'LainLainPrice' => $otherPrice, 'acteDtl' => $acteDtl, 'rjNo' => $rjNo],
            ['LainLainId' => 'bail|required|exists:rsmst_others,other_id', 'LainLainPrice' => 'bail|required|numeric', 'acteDtl' => 'bail|required|numeric', 'rjNo' => 'bail|required|numeric'],
        );
    ...
    {
        DB::table('rstxn_ugdothers')->where('acte_dtl', $rjActeDtl)->delete();
    }
    ...
    {
        $validator = Validator::make(
            ['productId' => $productId, 'qty' => $qty, 'productPrice' => $price, 'acteDtl' => $acteDtl, 'rjNo' => $rjNo],
            ['productId' => 'bail|required|exists:immst_products,product_id', 'qty' => 'bail|required|numeric|min:1', 'productPrice' => 'bail|required|numeric', 'acteDtl' => 'bail|required|numeric', 'rjNo' => 'bail|required|numeric'],
        );
    ...
    {
        DB::table('rstxn_ugdobats')->where('acte_dtl', $rjActeDtl)->delete();
    }
    ...
    public function resetFormEntry(): void
    {
        ...
        $this->incrementVersion('modal-jasa-karyawan-ugd');
    }
};
?>

<div class="space-y-4" wire:key="{{ $this->renderKey('modal-jasa-karyawan-ugd', [$rjNo ?? 'new']) }}">
    ...
    <div class="overflow-hidden bg-canvas border border-hairline rounded-2xl dark:border-gray-700 dark:bg-gray-900">
        <div class="flex items-center justify-between px-4 py-3 border-b border-hairline dark:border-gray-700">
            <h3 class="text-sm font-semibold text-body dark:text-gray-300">Daftar Jasa Karyawan</h3>
            <x-badge variant="gray">{{ count($ugdJasaKaryawan) }} item</x-badge>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-hairline-soft dark:divide-gray-800">
                    @forelse ($ugdJasaKaryawan as $item)
                        <tr wire:key="jasa-karyawan-ugd-{{ $item['rjActeDtl'] ?? $loop->index }}" class="transition group hover:bg-surface-soft dark:hover:bg-gray-800/40">
                            <td class="px-4 py-3 font-mono text-xs text-muted dark:text-gray-400 whitespace-nowrap">
                                {{ $item['JasaKaryawanId'] }}</td>
                            <td class="px-4 py-3 text-ink dark:text-gray-200 whitespace-nowrap">
                                {{ $item['JasaKaryawanDesc'] }}</td>
                            <td
                                class="px-4 py-3 font-semibold text-right text-ink dark:text-gray-200 whitespace-nowrap">
                                Rp {{ number_format($item['JasaKaryawanPrice']) }}</td>
                            @if (!$isFormLocked)
                    ...
                    <tfoot class="border-t border-hairline bg-surface-soft dark:bg-gray-800/50 dark:border-gray-700">
                        <tr>
                            ...
                            <td class="px-4 py-3 text-sm font-bold text-right text-ink dark:text-white">
                                Rp {{ number_format(collect($ugdJasaKaryawan)->sum('JasaKaryawanPrice')) }}
                            </td>
                            @if (!$isFormLocked)
                                <td></td>
                            @endif
