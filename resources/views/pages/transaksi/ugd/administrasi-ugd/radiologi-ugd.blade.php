...
new class extends Component {
    ...
    {
        $rows = DB::table('rstxn_ugdrads')
            ->join('rsmst_radiologis', 'rsmst_radiologis.rad_id', 'rstxn_ugdrads.rad_id')
            ->select('rstxn_ugdrads.rad_dtl', 'rstxn_ugdrads.rad_id', 'rsmst_radiologis.rad_desc', 'rstxn_ugdrads.rad_price')
            ->where('rj_no', $rjNo)
            ->orderBy('rstxn_ugdrads.rad_dtl')
            ->get();
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-hairline-soft dark:divide-gray-800">
                    @forelse ($rjRad as $item)
                        <tr wire:key="rad-ugd-row-{{ $item['radDtl'] }}"
