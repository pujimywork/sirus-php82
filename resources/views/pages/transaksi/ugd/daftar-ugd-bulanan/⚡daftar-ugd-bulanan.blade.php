...
new class extends Component {
    ...
    // (komponen anak di-unmount/remount oleh @if tab). Key di-namespace 'daftar-ugd-bulanan-*'.
    #[Session(key: 'daftar-ugd-bulanan-searchKeyword')]
    public string $searchKeyword = '';
    #[Session(key: 'daftar-ugd-bulanan-filterMode')]
    public string $filterMode = 'bulanan'; // 'bulanan' | 'harian'
    #[Session(key: 'daftar-ugd-bulanan-filterBulan')]
    public string $filterBulan = ''; // format m/Y (mm/yyyy) — dipakai mode bulanan
    #[Session(key: 'daftar-ugd-bulanan-filterTanggal')]
    public string $filterTanggal = ''; // format d/m/Y (dd/mm/yyyy) — dipakai mode harian
    #[Session(key: 'daftar-ugd-bulanan-filterStatus')]
    public string $filterStatus = 'L'; // default: Selesai
    #[Session(key: 'daftar-ugd-bulanan-filterKlaim')]
    public string $filterKlaim = 'BPJS'; // default: BPJS | '' | 'UMUM'
    #[Session(key: 'daftar-ugd-bulanan-filterPoli')]
    public string $filterPoli = '';
    #[Session(key: 'daftar-ugd-bulanan-filterDokter')]
    public string $filterDokter = '';
    #[Session(key: 'daftar-ugd-bulanan-itemsPerPage')]
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-hairline dark:divide-gray-700">
                            @forelse($this->rows as $row)
                                <tr wire:key="daftar-ugd-bulanan-{{ $row->rj_no ?? $loop->index }}" class="transition hover:bg-surface-soft dark:hover:bg-gray-800/50">
