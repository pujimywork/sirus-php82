<?php
// resources/views/pages/transaksi/casemix/rekap-idrg-ri/⚡rekap-idrg-ri.blade.php
// Rekap iDRG Rawat Inap (RI) — Casemix. Daftar pasien pulang (Bulanan/Harian) + tarik data klaim
// E-Klaim per SEP (get_claim_data). Kolom: No RM, Nama, DPJP, Diagnosa, ALOS, Cara
// Keluar, Tarif INA-CBG, Tarif RS. Loop getClaimData digerakkan Alpine sekuensial.
// Standarisasi toolbar + pagination mengikuti Daftar Pasien RJ — Casemix.
...
new class extends Component {
    ...
    protected array $renderAreas = ['rekap-idrg-ri-toolbar'];

    /* Filter & pagination state — pola sama dgn daftar-rj/ri-bulanan.
     | Dipersist per-tab (Session) supaya tak reset saat pindah tab di /transaksi/casemix
     | (komponen anak di-unmount/remount oleh @if tab). Key di-namespace 'rekap-idrg-ri-*'. */
    #[Session(key: 'rekap-idrg-ri-searchKeyword')]
    public string $searchKeyword = '';
    #[Session(key: 'rekap-idrg-ri-filterMode')]
    public string $filterMode = 'bulanan'; // 'bulanan' | 'harian'
    #[Session(key: 'rekap-idrg-ri-filterBulan')]
    public string $filterBulan = ''; // m/Y (mm/yyyy)
    #[Session(key: 'rekap-idrg-ri-filterTanggal')]
    public string $filterTanggal = ''; // d/m/Y (dd/mm/yyyy)
    #[Session(key: 'rekap-idrg-ri-filterStatus')]
    public string $filterStatus = 'P'; // default: Pulang
    #[Session(key: 'rekap-idrg-ri-filterKlaim')]
    public string $filterKlaim = 'BPJS'; // BPJS | UMUM | ''
    #[Session(key: 'rekap-idrg-ri-filterDokter')]
    public string $filterDokter = '';
    #[Session(key: 'rekap-idrg-ri-itemsPerPage')]
    ...
    public function updatedFilterMode(): void { $this->resetPage(); $this->claims = []; $this->incrementVersion('rekap-idrg-ri-toolbar'); }
    public function updatedFilterBulan(): void { $this->resetPage(); $this->claims = []; $this->incrementVersion('rekap-idrg-ri-toolbar'); }
    public function updatedFilterTanggal(): void { $this->resetPage(); $this->claims = []; $this->incrementVersion('rekap-idrg-ri-toolbar'); }
    public function updatedFilterStatus(): void { $this->resetPage(); $this->incrementVersion('rekap-idrg-ri-toolbar'); }
    public function updatedFilterKlaim(): void { $this->resetPage(); $this->incrementVersion('rekap-idrg-ri-toolbar'); }
    public function updatedFilterDokter(): void { $this->resetPage(); $this->incrementVersion('rekap-idrg-ri-toolbar'); }
    public function updatedItemsPerPage(): void { $this->resetPage(); $this->incrementVersion('rekap-idrg-ri-toolbar'); }
    ...
    public function resetFilters(): void
    {
        ...
        $this->claims = [];
        $this->incrementVersion('rekap-idrg-ri-toolbar');
        $this->resetPage();
    }

    private function dateRange(): array
    {
        // RI filter by exit_date (tgl pulang) — harian = 1 hari, bulanan = 1 bulan.
    ...
    <div class="w-full h-[calc(100vh-5rem)] flex flex-col bg-surface-soft dark:bg-gray-800">
        <div class="flex flex-col flex-1 min-h-0 px-6 pt-2 pb-6">
            ...
            <div class="sticky z-30 px-4 py-3 bg-surface-soft border-b border-hairline top-20 dark:bg-gray-900 dark:border-gray-700">
                <div class="flex flex-wrap items-end gap-3" wire:key="{{ $this->renderKey('rekap-idrg-ri-toolbar', []) }}">
                    ...
                    <div class="w-full sm:flex-1">
                        ...
                        <div class="relative mt-1">
                            ...
                            <x-text-input wire:model.live.debounce.300ms="searchKeyword" class="block w-full pl-10"
                                placeholder="Cari No RI / No RM / Nama Pasien / No SEP..." />
                        </div>
                    </div>
                    ...
                    {{-- FILTER BULAN / TANGGAL (Tgl Pulang) --}}
            ...
            <div class="mt-4 flex flex-col flex-1 min-h-0 bg-canvas border border-hairline shadow-sm rounded-2xl dark:border-gray-700 dark:bg-gray-900">
                ...
                <div class="flex flex-wrap gap-6 px-6 py-3 text-sm border-b border-hairline dark:border-gray-700">
                    <span class="text-muted dark:text-gray-300">Pasien (hal. ini): <b>{{ count($this->rows) }}</b></span>
