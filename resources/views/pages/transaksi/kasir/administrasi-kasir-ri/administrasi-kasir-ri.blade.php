...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-administrasi-kasir-ri'];
    ...
    public function open(int $slsNo, ?string $tab = null): void
    {
        ...
        $this->incrementVersion('modal-administrasi-kasir-ri');
        $this->dispatch('open-modal', name: 'administrasi-kasir-ri');
    }
    ...
    {
        $hdr = DB::table('imtxn_slshdrs as s')->join('rsmst_pasiens as p', 'p.reg_no', '=', 's.reg_no')->leftJoin('rsmst_doctors as d', 'd.dr_id', '=', 's.dr_id')->join('rstxn_rihdrs as r', 'r.rihdr_no', '=', 's.rihdr_no')->leftJoin('rsmst_klaimtypes as k', 'k.klaim_id', '=', 'r.klaim_id')->leftJoin('acmst_accounts as a', 'a.acc_id', '=', 's.acc_id')->select('s.sls_no', 's.status', 's.rihdr_no', 's.reg_no', 's.acte_price', 's.acc_id', 'a.acc_name', 's.sls_bayar', 's.sls_bon', DB::raw("to_char(s.sls_date,'dd/mm/yyyy hh24:mi:ss') as sls_date_display"), 'p.reg_name', 'p.sex', DB::raw("to_char(p.birth_date,'dd/mm/yyyy') as birth_date"), 'd.dr_name', 'r.ri_status', 'r.klaim_id', 'k.klaim_desc')->where('s.sls_no', $this->slsNo)->first();
    ...
    private function loadItems(): void
    {
        $this->items = DB::table('imtxn_slsdtls as dtl')
            ...
            ->select('dtl.sls_dtl', 'dtl.product_id', DB::raw('nvl(p.product_name,dtl.product_id) as product_name'), 'dtl.qty', 'dtl.sales_price', 'dtl.resep_carapakai', 'dtl.resep_kapsul', 'dtl.resep_takar', 'dtl.resep_ket', 'dtl.etiket_status', DB::raw("to_char(dtl.exp_date,'yyyy-mm-dd') as exp_date"))
    ...
    public function insertObat(): void
    {
        ...
        try {
            ...
            $this->recalcKasir();
            $this->resetFormEntry();
            $this->dispatch('focus-lov-obat-kasir-ri');
            $this->dispatch('toast', type: 'success', message: 'Obat berhasil ditambahkan.');
            $this->dispatch('refresh-after-antrian-kasir-ri.saved');
    ...
    public function removeObat(int $slsDtl): void
    {
        ...
        try {
            DB::transaction(function () use ($slsDtl) {
                $this->lockSlshdrAndGuard();
                DB::table('imtxn_slsdtls')->where('sls_dtl', $slsDtl)->where('sls_no', $this->slsNo)->delete();
            });
    ...
    /* ===============================
     | KASIR
     =============================== */
    #[On('lov.selected.kas-administrasi-kasir-ri')]
    ...
    public function postTransaksi(): void
    {
        ...
        try {
            DB::transaction(function () use ($bayar, $totalAll, $isBon, $sisa, $empId) {
                ...
                $shift = DB::table('rstxn_shiftctls')->select('shift')->whereRaw("to_char(sysdate,'HH24:MI:SS') between shift_start and shift_end")->value('shift') ?? ($current->shift ?? 1);
            ...
            $msg = $isBon ? 'Transaksi tersimpan. Sisa Rp ' . number_format($totalAll - $bayar) . ' masuk Bon Inap.' : 'Transaksi LUNAS tersimpan.';
    ...
    {
        $this->dispatch('close-modal', name: 'administrasi-kasir-ri');
        $this->resetForm();
    }
    ...
    {
        $this->reset(['slsNo', 'rihdrNo', 'isLoaded', 'activeTab', 'regNo', 'regName', 'sex', 'birthDate', 'drName', 'klaimId', 'klaimDesc', 'riStatus', 'slsDateDisplay', 'status', 'items', 'editingDtl', 'editRow', 'bayar', 'kembalian', 'kekurangan', 'accId', 'accName']);
        ...
        <div wire:key="{{ $this->renderKey('modal-administrasi-kasir-ri', [$slsNo ?? 'new']) }}" x-data
                    ...
                    <div
                        ...
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2M12 11h4m-4 4h4m-6-4h.01M10 15h.01" />
                        </svg>
                    </div>
                    <div class="min-w-0">
                ...
                <div class="flex items-center gap-3 shrink-0">
                    ...
                    @if ($isLoaded)
                        <div class="flex gap-2">
                            ...
                            <div
                                class="px-3 py-1.5 bg-amber-50 border border-amber-200 rounded-lg dark:bg-amber-900/20 dark:border-amber-700">
                            ...
                            <div
                                class="px-3 py-1.5 bg-blue-50 border border-blue-200 rounded-lg dark:bg-blue-900/20 dark:border-blue-700">
            ...
            @else
                ...
                @if ($activeTab === 'kasir')
                    ...
                    <div class="px-6 py-4 max-h-[calc(100vh-380px)] overflow-y-auto space-y-4"
                        wire:key="{{ $this->renderKey('modal-administrasi-kasir-ri', [$slsNo ?? 'new']) }}-kasir-tab">
                                    ...
                                    @if ($sisaTagihan > 0)
                                        <div
                                            ...
                                            <div>
                                                ...
                                                <p class="mt-0.5">Sisa Rp {{ number_format($sisaTagihan) }} ditagih
                                                    saat pasien pulang.</p>
                                            </div>
                                        </div>
                                    @else
                                        <div
                                            ...
                                            <div>
                                                ...
                                                <p class="mt-0.5">Pembayaran sudah selesai. Klik "Batal Transaksi" jika
                                                    perlu membatalkan.</p>
                                            </div>
                                        </div>
                                    @endif
                ...
                <div
                    ...
                    <div class="flex gap-2">
                        @if ($activeTab === 'kasir' && $this->isKasirPosted)
                            <x-info-button wire:click="cetakKwitansi" wire:loading.attr="disabled"
                                wire:target="cetakKwitansi">
